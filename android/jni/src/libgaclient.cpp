...
jobject initAudio(JNIEnv* jnienv, const char* mime, int sampleRate, int channelCount, bool builtinDecoder)
{
	...
	obj	= env->CallObjectMethod(
		 g_obj, g_mid_initAudio, jmime, (jint)sampleRate, (jint)channelCount, builtinDecoder ? JNI_TRUE : JNI_FALSE);
	return obj;
}
...
int decodeAudio(JNIEnv* jnienv, unsigned char* data, int len, struct timeval pts, int flags)
{
	...
	env->SetByteArrayRegion(jaudioBuf, (jsize)0, (jsize)len, (jbyte*)data);
	return env->CallIntMethod(g_obj, g_mid_decodeAudio, jaudioBuf, (jint)len, (jlong)pts.tv_sec * 1000000 + pts.tv_usec, flags);
}
...
jobject initVideo(JNIEnv* jnienv, const char* mime, int width, int height)
{
	...
	if((env = attachThread(jnienv)) == NULL)
		return NULL;
	jmime = env->NewStringUTF(mime);
	obj	= env->CallObjectMethod(g_obj, g_mid_initVideo, jmime, (jint)width, (jint)height);
	return obj;
}
...
jobject videoSetByteBuffer(JNIEnv* jnienv, const char* name, unsigned char* value, int len)
{
	...
	env->SetByteArrayRegion(jvalue, (jsize)0 /*start*/, (jsize)len /*len*/, (jbyte*)value);
	obj = env->CallObjectMethod(g_obj, g_mid_videoSetByteBuffer, jname, jvalue, (jint)len);
...
int decodeVideo(JNIEnv* jnienv, unsigned char* data, int len, struct timeval pts, bool marker, int flags)
{
	...
	env->SetByteArrayRegion(jvideoBuf, (jsize)0, (jsize)len, (jbyte*)data);
	return env->CallIntMethod(g_obj,
									  g_mid_decodeVideo,
									  jvideoBuf,
									  (jint)len,
...
void gl_resize(int width, int height)
{
	...
	if(img_width <= 0 || img_height <= 0 || gl_width <= 0 || gl_height <= 0)
	{
...
JNIEXPORT jboolean JNICALL Java_org_gaminganywhere_gaclient_GAClient_initGAClient(JNIEnv* env, jobject thisObj, jobject thiz)
{
	jclass localClass;
	//
	...
	LOAD_METHOD(initAudio, "initAudio", "(Ljava/lang/String;IIZ)Ljava/lang/Object;");
	LOAD_METHOD(startAudioDecoder, "startAudioDecoder", "()Landroid/media/MediaCodec;");
	LOAD_METHOD(decodeAudio, "decodeAudio", "([BIJI)I");
	LOAD_METHOD(initVideo, "initVideo", "(Ljava/lang/String;II)Landroid/media/MediaFormat;");
	LOAD_METHOD(videoSetByteBuffer, "videoSetByteBuffer", "(Ljava/lang/String;[BI)Landroid/media/MediaFormat;");
	LOAD_METHOD(startVideoDecoder, "startVideoDecoder", "()Landroid/media/MediaCodec;");
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_setBuiltinAudioInternal(JNIEnv* env,
																													  jobject thisObj,
																													  jboolean enable)
{
	if(enable != JNI_FALSE)
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_setBuiltinVideoInternal(JNIEnv* env,
																													  jobject thisObj,
																													  jboolean enable)
{
	if(enable != JNI_FALSE)
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_setAudioCodec(JNIEnv* env,
																										 jobject thisObj,
																										 /*jstring codecname,*/ jint samplerate,
																										 jint channels)
{
	//
#if 0
	...
	ga_log(
	  "setAudioCodec: %s, samplerate=%d, channels=%d\n", "codec auto-detect", g_conf->audio_samplerate, g_conf->audio_channels);
#if 0
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_setDropLateVideoFrame(JNIEnv* env, jobject thisObj, jint ms)
{
	char value[16] = "-1";
	if(ms > 0)
	{
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_sendKeyEvent(JNIEnv* env,
																										jobject thisObj,
																										jboolean pressed,
																										jint scancode,
																										jint sym,
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_sendMouseKey(JNIEnv* env,
																										jobject thisObj,
																										jboolean pressed,
																										jint button,
																										jint x,
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_sendMouseMotion(JNIEnv* env,
																											jobject thisObj,
																											jint x,
																											jint y,
																											jint xrel,
...
static int launch_controller_client(JNIEnv* env)
{
...
static void disconnect(JNIEnv* env)
{
	//
...
static void rtspConnect_cleanup(JNIEnv* env)
{
	int i;
	if(jaudioBuf != NULL)
	{
...
JNIEXPORT void JNICALL Java_org_gaminganywhere_gaclient_GAClient_GLresize(JNIEnv* env, jobject thisObj, jint width, jint height)
{
...
JNIEXPORT jboolean JNICALL Java_org_gaminganywhere_gaclient_GAClient_GLrenderInternal(JNIEnv* env, jobject thisObj)
{
	if(gl_render() < 0)
		return JNI_FALSE;
	return JNI_TRUE;
...
JNIEXPORT jint JNICALL Java_org_gaminganywhere_gaclient_GAClient_audioBufferFill(JNIEnv* env,
																											jobject thisObj,
																											jbyteArray stream,
																											jint size)
{
	...
	if(fillsize > 0)
	{
		env->SetByteArrayRegion(stream, (jsize)0, (jsize)fillsize, (jbyte*)tmpbuf);
	}
	return fillsize;
}
...
JNIEXPORT jboolean JNICALL Java_org_gaminganywhere_gaclient_GAClient_rtspConnect(JNIEnv* env, jobject thisObj)
{
	char urlbuf[2048];
	//
	if(g_conf->servername[0] == '\0' || g_conf->object[0] == '\0')
	{
		ga_log("connect: No host or objpath given.\n");
		return JNI_FALSE;
	}
	snprintf(urlbuf, sizeof(urlbuf), "rtsp://%s:%d%s", g_conf->servername, g_conf->serverport, g_conf->object);
	ga_log("Target URL[%s]\n", urlbuf);
	//
#if 0
