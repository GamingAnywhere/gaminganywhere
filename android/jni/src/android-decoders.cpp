...
static unsigned char* android_config_h264_internal(unsigned char* old, unsigned char* buf, int buflen, int* outlen)
{
	unsigned char* conf;
...
int android_config_h264_sprop(RTSPThreadParam* rtspParam, const char* sprop)
{
	...
	if(sprop == NULL)
		return -1;
	//
	if(config_h264_sps != NULL)
		free(config_h264_sps);
	if(config_h264_pps != NULL)
		free(config_h264_pps);
		...
		*s1 = '\0';
		if((blen = av_base64_decode((u_int8_t*)&frame[sizeof(startcode)], (const char*)s0, sizeof(frame) - sizeof(startcode))) > 0)
		{
			...
			if(frame[sizeof(startcode)] == 0 && frame[sizeof(startcode) + 1] == 0)
			{
				...
				else if(frame[sizeof(startcode) + 2] == 0 && frame[sizeof(startcode) + 3] == 1)
				{
					framing			 = 4;
					minus_startcode = 4;
				}
			...
			switch(frame[sizeof(startcode) + framing] & 0x1f)
			{
				case 7: // SPS
					config_h264_sps = android_config_h264_internal(
					  config_h264_sps, frame + minus_startcode, sizeof(startcode) + blen - minus_startcode, &config_h264_spslen);
					if(config_h264_sps)
					{
				...
				case 8: // PPS
					config_h264_pps = android_config_h264_internal(
					  config_h264_pps, frame + minus_startcode, sizeof(startcode) + blen - minus_startcode, &config_h264_ppslen);
					if(config_h264_pps)
					{
...
static int android_start_h264(RTSPThreadParam* rtspParam)
{
