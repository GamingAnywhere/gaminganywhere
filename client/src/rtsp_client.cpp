...
void bandwidth_estimator_update(unsigned int ssrc,
										  unsigned short seq,
	...
	if((seq - 1) == mi->second.lastPktSeq)
	{
		if(sampleframe)
		{
			...
			if(mi->second.framecount >= 240 && mi->second.samples >= 3000)
			{
				...
				ga_error("XXX: received: %uKB; capacity = %.3fKbps (%d samples); loss-rate = %.2f%% (%u/%u); average per-frame "
							"overhead factor = %.2f\n",
...
static int play_video_priv(int ch /*channel*/, unsigned char* buffer, int bufsize, struct timeval pts)
{
	...
	while(avpkt.size > 0)
	{
		...
		if(got_picture)
		{
			...
			// do scaling
			if(vframe[ch]->width == rtspParam->width[ch] && vframe[ch]->height == rtspParam->height[ch] &&
			...
			else
			{
				...
				SwsContext* swsctx;
				if((swsctx = create_frame_converter(vframe[ch]->width,
																vframe[ch]->height,
	...
	else
	{
		...
		if(pts.tv_sec != pdb->lastpts.tv_sec || pts.tv_usec != pdb->lastpts.tv_usec)
		{
			if(pdb->privbuflen > 0)
			{
				...
				left = play_video_priv(channel, pdb->privbuf, pdb->privbuflen, pdb->lastpts);
				if(left > 0)
				{
					bcopy(pdb->privbuf + pdb->privbuflen - left, pdb->privbuf, left);
					pdb->privbuflen = left;
		...
		else
		{
			...
			if(swrctx == NULL)
			{
				...
				rtsperror("audio decoder: convert from %dch(%x)@%dHz (%s) to %dch(%x)@%dHz (%s).\n",
							 (int)aframe->channels,
							 (int)aframe->channel_layout,
							 (int)aframe->sample_rate,
			...
			datalen = av_samples_get_buffer_size(
			  NULL, rtspconf->audio_channels, aframe->nb_samples, rtspconf->audio_device_format, 1 /*no-alignment*/);
...
void* rtsp_thread(void* param)
{
	...
	if(savefp_yuv != NULL && ga_conf_readv("save-yuv-image-timestamp", savefile_yuvts, sizeof(savefile_yuvts)) != NULL)
	...
	audio_sess_fmt = -1;
	if(video_codec_name != NULL)
		free((void*)video_codec_name);
	if(audio_codec_name != NULL)
		free((void*)audio_codec_name);
...
{
	RTSPClient* rtspClient = ourRTSPClient::createNew(env, rtspURL, RTSP_CLIENT_VERBOSITY_LEVEL, "RTSP Client" /*"rtsp_thread"*/);
	if(rtspClient == NULL)
	{
	...
	do
		if(scs.subsession != NULL)
		{
			...
			else
			{
				...
				env << *rtspClient << "Initiated the \"" << *scs.subsession << "\" subsession (client ports "
					 << scs.subsession->clientPortNum() << "-" << scs.subsession->clientPortNum() + 1 << ")\n";

				// Continue setting up this subsession, by sending a RTSP "SETUP" command:
				rtspClient->sendSetupCommand(
				  *scs.subsession, continueAfterSETUP, False, rtpOverTCP ? True : False /*TCP?*/, False, NULL);
			}
			return;
		}
	while(0);
...
static void NATHolePunch(RTPSource* rtpsrc, MediaSubsession* subsession)
{
...
#endif
	if(rtspconf->sin.sin_addr.s_addr == 0 || rtspconf->sin.sin_addr.s_addr == INADDR_NONE)
	{
	...
	sendto(s, (const char*)buf, 1, 0, (struct sockaddr*)&sin, sizeof(sin));
	usleep(5000);
	sendto(s, (const char*)buf, 1, 0, (struct sockaddr*)&sin, sizeof(sin));
	usleep(5000);
	sendto(s, (const char*)buf, 1, 0, (struct sockaddr*)&sin, sizeof(sin));
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString)
{
	do
	{
		...
		env << *rtspClient << "Set up the \"" << *scs.subsession << "\" subsession (client ports "
			 << scs.subsession->clientPortNum() << "-" << scs.subsession->clientPortNum() + 1 << ")\n";
...
void shutdownStream(RTSPClient* rtspClient, int exitCode)
{
	...
	if(--rtspClientCount == 0)
	{
		...
		// and replace it with "eventLoopWatchVariable = 1;", so that we leave the LIVE555 event loop, and continue running
		// "main()".)
		// exit(exitCode);
	...
	MediaSink(env), fSubsession(subsession)
{
	...
	// H264 framing code
	if(strcmp("H264", fSubsession.codecName()) == 0 || strcmp("H265", fSubsession.codecName()) == 0)
	{
		video_framing														 = 4;
		fReceiveBuffer[MAX_FRAMING_SIZE - video_framing + 0]	 = fReceiveBuffer[MAX_FRAMING_SIZE - video_framing + 1] =
...
Boolean DummySink::continuePlaying()
{
	...
	fSource->getNextFrame(
	  fReceiveBuffer + MAX_FRAMING_SIZE, DUMMY_SINK_RECEIVE_BUFFER_SIZE, afterGettingFrame, this, onSourceClosure, this);
	return True;
}
