...
static unsigned int h264_buf_ue(struct bufinfo* bi)
{
	unsigned int val = 0;
	unsigned int b;
	int leading = -1;
...
static int h264_map_se(unsigned int code)
{
	int sign = 1;
	if((code & 0x01) == 0)
		sign = -1;
...
static int parse_sps(struct mini_h264_context* ctx, unsigned char* buf, int len)
{
	...
	ctx->width	= ((sps->pic_width_in_mbs_minus1 + 1) * 16) - sps->frame_crop_right_offset * 2 - sps->frame_crop_left_offset * 2;
	ctx->height = ((2 - sps->frame_mbs_only_flag) * (sps->pic_height_in_map_units_minus1 + 1) * 16) -
