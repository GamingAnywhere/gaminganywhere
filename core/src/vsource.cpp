...
#define COLORCODE_SUFFIX                      \
	(COLORCODE_CRC + COLORCODE_ID) /**< Digits \
 ...
 */
vsource_frame_t* vsource_frame_init(int channel, vsource_frame_t* frame)
{
	int i;
	vsource_t* vs;
	//
 ...
 */
void vsource_frame_release(vsource_frame_t* frame) { return; }
...
static unsigned char rgbacolor[8][4] = {{0, 0, 0, 255},
													 {0, 0, 234, 255},
													 {0, 234, 0, 255},
													 {234, 0, 0, 255},
													 {234, 234, 0, 255},
 ...
 * The order is: blak blue green, red, yellow, magenta, cyan, and white */
static unsigned char bgracolor[8][4] = {{0, 0, 0, 255},
													 {234, 0, 0, 255},
													 {0, 234, 0, 255},
													 {0, 0, 234, 255},
													 {0, 234, 234, 255},
...
static unsigned char
  vsource_colorcode_buffer[4 * COLORCODE_MAX_WIDTH * (COLORCODE_MAX_DIGIT + COLORCODE_SUFFIX)]; /**<
...
int vsource_embed_colorcode_init(int RGBmode)
{
	...
	vsource_colorcode_total_width	 = (vsource_colorcode_digits + COLORCODE_SUFFIX) * vsource_colorcode_width;
	...
	//
	ga_error(
	  "video source: color code initialized - %dx%d, %d digits, shift=%d, initmask=%08x, totalwidth=%d, counter-mask=%08x\n",
	  vsource_colorcode_width,
	  vsource_colorcode_height,
	  vsource_colorcode_digits,
	  vsource_colorcode_initshift,
	  vsource_colorcode_initmask,
...
static void vsource_embed_yuv_code(vsource_frame_t* frame, unsigned int value)
{
	...
	height = frame->realheight < vsource_colorcode_height ? frame->realheight : vsource_colorcode_height;
	dstY	 = frame->imgbuf;
 ...
 */
int video_source_channels() { return gChannels; }
...
int video_source_setup_ex(vsource_config_t* config, int nConfig)
{
	...
	for(idx = 0; idx < nConfig; idx++)
	{
		...
		// create pipe
		gPipe[idx] = dpipe_create(
		  idx, pipename, VIDEO_SOURCE_POOLSIZE, sizeof(vsource_frame_t) + vs->max_height * vs->max_stride + VSOURCE_ALIGNMENT);
		if(gPipe[idx] == NULL)
		{
