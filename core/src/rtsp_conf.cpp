...
struct RTSPConf* rtspconf_global() { return &globalConf; }
...
static int rtspconf_load_codec(const char* key,
										 const char* value,
										 const char** names,
										 AVCodec** codec,
										 AVCodec* (*finder)(const char**, enum AVCodecID))
{
	//
	int idx = 0;
	...
	//
	ga_error("# RTSP[config]: %s = %s (%s)\n", key, (*codec)->name, (*codec)->long_name == NULL ? "N/A" : (*codec)->long_name);
	return 0;
}
...
int rtspconf_parse(struct RTSPConf* conf)
{
	...
	{
		if(rtspconf_load_codec(
			  "video-encoder", ptr, (const char**)conf->video_encoder_name, &conf->video_encoder_codec, ga_avcodec_find_encoder) < 0)
			return -1;
	}
#if 0
	...
	{
		if(rtspconf_load_codec(
			  "audio-encoder", ptr, (const char**)conf->audio_encoder_name, &conf->audio_encoder_codec, ga_avcodec_find_encoder) < 0)
			return -1;
	}
#if 0
	...
	if(ga_conf_mapsize("video-specific") > 0)
	{
		...
		for(ptr = ga_conf_mapkey("video-specific", buf, sizeof(buf)); ptr != NULL;
