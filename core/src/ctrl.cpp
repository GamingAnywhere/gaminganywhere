...
sdlmsg_t* sdlmsg_ntoh(sdlmsg_t* msg)
{
	...
	switch(msg->msgtype)
	{
		case SDL_EVENT_MSGTYPE_KEYBOARD:
			if(msgk->scancode)
				msgk->scancode = ntohs(msgk->scancode);
			if(msgk->sdlkey)
				msgk->sdlkey = (int)ntohl(msgk->sdlkey);
			if(msgk->unicode)
				msgk->unicode = ntohl(msgk->unicode);
			if(msgk->sdlmod)
				msgk->sdlmod = ntohs(msgk->sdlmod);
			break;
		case SDL_EVENT_MSGTYPE_MOUSEKEY:
		case SDL_EVENT_MSGTYPE_MOUSEMOTION:
		case SDL_EVENT_MSGTYPE_MOUSEWHEEL:
			if(msgm->mousex)
				msgm->mousex = ntohs(msgm->mousex);
			if(msgm->mousey)
				msgm->mousey = ntohs(msgm->mousey);
			if(msgm->mouseRelX)
				msgm->mouseRelX = ntohs(msgm->mouseRelX);
			if(msgm->mouseRelY)
				msgm->mouseRelY = ntohs(msgm->mouseRelY);
			break;
	}
#if 0
#if 1 // only support SDL2
...
sdlmsg_t* sdlmsg_keyboard(sdlmsg_t* msg,
...
sdlmsg_t* sdlmsg_mousemotion(sdlmsg_t* msg,
	...
	switch(msg->msgtype)
	{
		...
		case SDL_EVENT_MSGTYPE_MOUSEMOTION:
			...
			if(msgm->relativeMouseMode == 0)
			{
				if(prect == NULL)
				{
					in.mi.dx = (DWORD)(65536.0 * scaleFactorX * msgm->mousex) / cxsize;
					in.mi.dy = (DWORD)(65536.0 * scaleFactorY * msgm->mousey) / cysize;
				}
				else
				{
					in.mi.dx = (DWORD)(65536.0 * (prect->left + scaleFactorX * msgm->mousex)) / cxsize;
					in.mi.dy = (DWORD)(65536.0 * (prect->top + scaleFactorY * msgm->mousey)) / cysize;
				}
...
static void sdlmsg_replay_native(sdlmsg_t* msg)
{
	...
	switch(msg->msgtype)
	{
		case SDL_EVENT_MSGTYPE_KEYBOARD:
			...
			if((ksym = SDLKeyToKeySym(msgk->sdlkey)) != INVALID_KEY)
			{
				...
				if((kcode = XKeysymToKeycode(display, ksym)) > 0)
				{
					...
					XTestFakeKeyEvent(display, kcode, msgk->is_pressed ? True : False, CurrentTime);
					XSync(display, True);
		...
		case SDL_EVENT_MSGTYPE_MOUSEKEY:
			// ga_error("sdl replayer: button event btn=%u pressed=%d\n", msg->mousebutton, msg->is_pressed);
			XTestGrabControl(display, True);
			XTestFakeButtonEvent(display, msgm->mousebutton, msgm->is_pressed ? True : False, CurrentTime);
			XSync(display, True);
			XTestGrabControl(display, False);
			break;
		case SDL_EVENT_MSGTYPE_MOUSEWHEEL:
		...
		case SDL_EVENT_MSGTYPE_MOUSEMOTION:
			...
			if(prect == NULL)
			{
				XTestFakeMotionEvent(
				  display, screenNumber, (int)(scaleFactorX * msgm->mousex), (int)(scaleFactorY * msgm->mousey), CurrentTime);
			}
			else
			{
				XTestFakeMotionEvent(display,
											...
											(int)(prect->top + scaleFactorY * msgm->mousey),
											CurrentTime);
			}
			XSync(display, True);
			XTestGrabControl(display, False);
...
int sdlmsg_kb_init()
{
	...
	if(ga_conf_mapsize("key-block-scancode") > 0)
	{
		...
		for(key = ga_conf_mapkey("key-block-scancode", keybuf, sizeof(keybuf)); key != NULL;
	...
	if(ga_conf_mapsize("key-block-keycode") > 0)
	{
		...
		for(key = ga_conf_mapkey("key-block-keycode", keybuf, sizeof(keybuf)); key != NULL;
