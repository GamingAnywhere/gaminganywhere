...
static void vpu_encoder_free_phymem(vpu_context_t* ctx)
{
	int i;
	vpu_mem_desc desc;
	//
...
static int vpu_encoder_alloc_phymem(vpu_context_t* ctx, vpu_mem_desc* desc, int alignment)
{
	int ret;
	vpu_mem_desc tmp;
	//
...
static int vpu_encoder_alloc_framebuffer1(vpu_context_t* ctx,
														FrameBuffer* fb,
														int index,
...
static int vpu_encoder_alloc_framebuffer(vpu_context_t* ctx)
{
	int i;
	if(ctx == NULL)
		return -1;
...
static int vpu_encoder_retrieve_h264_param(vpu_context_t* ctx, int type, unsigned char* buf, int buflen)
{
...
static int vpu_encoder_retrieve_h264_spspps(vpu_context_t* ctx)
{
	int len;
	//
	if(ctx == NULL)
...
int vpu_encoder_init(vpu_context_t* ctx, int width, int height, int fps_n, int fps_d, int bitrate, int gopsize)
{
	...
	ret										= vpu_EncRegisterFrameBuffer(ctx->vpu_handle,
												  ctx->vpu_frames,
												  ctx->vpu_nframes - 2,
												  ctx->vpu_phy_width,
												  ctx->vpu_width,
	...
	// alloc input buffer
	if(vpu_encoder_alloc_framebuffer1(ctx, &ctx->vpu_InBuffer, ctx->vpu_nframes, &ctx->vpu_InBufferPhy, &ctx->vpu_InBufferVirt) <
		0)
	{
...
int main(int argc, char* argv[])
{
	...
	ga_error("%d frames processed in %.4f sec, fps = %.4f\n", framecount, 0.000001 * elapsed, 1000000.0 * framecount / elapsed);
	return 0;
}
#endif /* ! GA_MODULE */
