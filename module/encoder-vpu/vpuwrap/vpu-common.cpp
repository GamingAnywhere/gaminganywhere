...
static void vpu_encoder_free_memories(EncMemInfo* emi)
{
	int i;
	VpuEncRetCode ret;
	VpuMemDesc desc;
...
static int vpu_encoder_alloc_memories(VpuMemInfo* mi, EncMemInfo* emi)
{
	VpuEncRetCode ret;
	int i, size;
...
static int vpu_encoder_alloc_framebuffer(int n, int width, int height, EncMemInfo* emi, int nAlignment)
{
	int i;
	VpuEncRetCode ret;
...
static int vpu_encoder_h264_spspps()
{
#if 0
...
unsigned char* vpu_encoder_encode(unsigned char* frame, int framesize, int* encsize)
{
	...
	// has output?
	if((ep.eOutRetCode & VPU_ENC_OUTPUT_DIS) || (ep.eOutRetCode & VPU_ENC_OUTPUT_SEQHEADER))
	{
