...
static int check_wave_format(ga_wasapi_param* wparam)
{
...
int ga_wasapi_init(ga_wasapi_param* wasapi)
{
	...
	//
	hr = CoCreateInstance(CLSID_MMDeviceEnumerator, NULL, CLSCTX_ALL, IID_IMMDeviceEnumerator, (void**)&wasapi->pEnumerator);
	...
	hr = wasapi->pDevice->Activate(IID_IAudioClient, CLSCTX_ALL, NULL, (void**)&wasapi->pAudioClient);
	...
	hr = wasapi->pAudioClient->Initialize(
	  AUDCLNT_SHAREMODE_SHARED, AUDCLNT_STREAMFLAGS_LOOPBACK /*0*/, hnsRequestedDuration, 0, wasapi->pwfx, NULL);
	...
	wasapi->hnsActualDuration = (UINT32)((double)REFTIMES_PER_SEC * wasapi->bufferFrameCount / wasapi->pwfx->nSamplesPerSec);
	wasapi->bufferFillInt	  = (DWORD)(wasapi->hnsActualDuration / REFTIMES_PER_MILLISEC / 2);

	hr = wasapi->pAudioClient->GetService(IID_IAudioCaptureClient, (void**)&wasapi->pCaptureClient);
...
int ga_wasapi_read(ga_wasapi_param* wasapi, unsigned char* wbuf, int wframes)
{
	...
	if(wasapi->firstRead.tv_sec == 0)
	{
		...
		wasapi->trimmedFrames =
		  (UINT64)(1.0 * wasapi->samplerate * tvdiff_us(&wasapi->firstRead, &wasapi->initialTimestamp) / 1000000);
	...
	if(packetLength == 0)
	{
		...
		{
			// fill silence
			double silenceFrame = 1.0 * tvdiff_us(&afterSleep, &wasapi->silenceFrom) * wasapi->samplerate / 1000000.0;
	...
	{
		hr = wasapi->pCaptureClient->GetBuffer(&pData, &numFramesAvailable, &flags, &framePos, NULL);
