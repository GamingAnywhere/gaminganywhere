...
static int filter_RGB2YUV_init(void* arg)
{
	...
	for(iid = 0; iid < video_source_channels(); iid++)
	{
		...
		if(ga_conf_readv("filter-source-pixelformat", pixelfmt, sizeof(pixelfmt)) != NULL)
		{
			...
			{
				swsctx = create_frame_converter(inputW, inputH, AV_PIX_FMT_RGBA, outputW, outputH, AV_PIX_FMT_YUV420P);
				ga_error("RGB2YUV filter: RGBA source specified.\n");
			}
			else if(strcasecmp("bgra", pixelfmt) == 0)
			{
				swsctx = create_frame_converter(inputW, inputH, AV_PIX_FMT_BGRA, outputW, outputH, AV_PIX_FMT_YUV420P);
				ga_error("RGB2YUV filter: BGRA source specified.\n");
			}
			else if(strcasecmp("yuv420p", pixelfmt) == 0)
			{
				swsctx = create_frame_converter(inputW, inputH, AV_PIX_FMT_YUV420P, outputW, outputH, AV_PIX_FMT_YUV420P);
		...
		//
		dstpipe[iid] = dpipe_create(iid, dstpipename, POOLSIZE, sizeof(vsource_frame_t) + video_source_mem_size(iid));
		if(dstpipe[iid] == NULL)
		{
...
static int filter_RGB2YUV_deinit(void* arg)
{
	if(savefp != NULL)
	{
...
static void* filter_RGB2YUV_threadproc(void* arg)
{
	...
	while(filter_started != 0)
	{
		...
		swsctx = lookup_frame_converter(srcframe->realwidth,
												  srcframe->realheight,
		...
		if(swsctx == NULL)
		{
			swsctx = create_frame_converter(srcframe->realwidth,
													  srcframe->realheight,
													  srcframe->pixelformat,
													  dstframe->realwidth,
													  dstframe->realheight,
		...
		//
		if(srcframe->pixelformat == AV_PIX_FMT_RGBA || srcframe->pixelformat == AV_PIX_FMT_BGRA /*rgba*/)
		{
...
static int filter_RGB2YUV_start(void* arg)
{
	int iid;
	const char** filterpipe = (const char**)arg;
...
static int filter_RGB2YUV_stop(void* arg)
{
	int iid;
	if(filter_started == 0)
		return 0;
