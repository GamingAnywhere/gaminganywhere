...
static void block_until_flushed(omx_streamer_t* ctx)
{
	int quit;
	while(!quit)
	{
...
static void init_component_handle(const char* name, OMX_HANDLETYPE* hComponent, OMX_PTR pAppData, OMX_CALLBACKTYPE* callbacks)
{
	OMX_ERRORTYPE r;
	char fullname[32];
...
static void dump_event(OMX_HANDLETYPE hComponent, OMX_EVENTTYPE eEvent, OMX_U32 nData1, OMX_U32 nData2)
{
	...
	ga_error(
	  "*** Received event 0x%08x %s, hComponent:0x%08x, nData1:0x%08x, nData2:0x%08x\n", eEvent, e, hComponent, nData1, nData2);
	return;
}
...
static OMX_ERRORTYPE fill_output_buffer_done_handler(OMX_HANDLETYPE hComponent, OMX_PTR pAppData, OMX_BUFFERHEADERTYPE* pBuffer)
{
	//
	omx_streamer_t* ctx = ((omx_streamer_t*)pAppData);
	vcos_semaphore_wait(&ctx->handler_lock);
...
static int omx_streamer_deinit_streamer(omx_streamer_t* ctx)
{
	OMX_ERRORTYPE r;
	/////////////////////////////////////
...
static void omx_streamer_default_config(omx_streamer_t* ctx)
{
	if(ctx == NULL)
		return;
...
static int omx_streamer_init_streamer(omx_streamer_t* ctx,
												  ...
												  int gopsize)
{
	...
	camera_portdef.format.video.nStride =
	  (camera_portdef.format.video.nFrameWidth + camera_portdef.nBufferAlignment - 1) & (~(camera_portdef.nBufferAlignment - 1));
	...
	ga_error("streamer: AVC config - nSliceHeaderSpacing=%d nPFrames=%d nBFrames=%d nRefFrames=%d UEP=%d FMO=%d ASO=%d RS=%d "
				"Profile=0x%x Level=0x%x AllowedFrames=0x%x\n",
...
static int omx_streamer_dummy_encode(omx_streamer_t* ctx)
{
	OMX_ERRORTYPE r;
	//
...
int omx_streamer_init(omx_streamer_t* ctx,
							 omx_streamer_config_t* config,
							 int width,
							 int height,
							 int fps_n,
...
int omx_streamer_resume(omx_streamer_t* ctx)
{
	if(psuspend == NULL)
		return 0;
	if(omx_streamer_init_streamer(ctx,
											&psuspend->config,
											psuspend->width,
											psuspend->height,
											psuspend->fps_n,
...
int omx_streamer_reconfigure(omx_streamer_t* ctx,
									  int bitrateKbps,
									  ...
									  unsigned int height)
{
	...
	omx_streamer_deinit_streamer(ctx);
	if(omx_streamer_init_streamer(ctx, &old_config, old_width, old_height, fps_n, fps_d, bitrate, old_gopsize) < 0)
	{
...
static int omx_streamer_start_stop(omx_streamer_t* ctx, OMX_BOOL start)
{
	OMX_ERRORTYPE r;
...
int omx_streamer_start(omx_streamer_t* ctx) { return omx_streamer_start_stop(ctx, OMX_TRUE); }

int omx_streamer_stop(omx_streamer_t* ctx) { return omx_streamer_start_stop(ctx, OMX_FALSE); }
...
void do_quit(int s) { quit_flag = 1; }
