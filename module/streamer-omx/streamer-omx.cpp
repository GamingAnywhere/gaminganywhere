...
static void camera_control(void* msg, int msglen)
{
	...
	if(m->is_pressed == 0)
	{
		...
		char cmd[512];
		struct stat s;
		snprintf(filename, sizeof(filename), "%s/%s%04d.%s", photo_dir, PHOTO_PREFIX, photo_seq++, PHOTO_EXT);
		snprintf(cmd, sizeof(cmd), "/usr/bin/raspistill -n -w 2592 -h 1944 -q 95 -e jpg -o %s", filename);
...
static int gaomx_load_int(const char* name, int min, int max, int def)
{
	char *ptr, value[64];
	int rv;
...
static OMX_BOOL gaomx_load_bool(const char* name, OMX_BOOL def)
{
	char *ptr, value[64];
...
static int vencoder_init(void* arg)
{
	...
	sc.camera_whitebalance			= (enum OMX_WHITEBALCONTROLTYPE)gaomx_load_int(
		"camera-omx-whitebalance", 0, (int)OMX_WhiteBalControlMax, (int)OSCAM_DEF_WHITE_BALANCE_CONTROL);
	sc.camera_filter = (enum OMX_IMAGEFILTERTYPE)gaomx_load_int(
	  "camera-omx-filter", (int)OMX_ImageFilterNone, (int)OMX_ImageFilterMax, (int)OSCAM_DEF_IMAGE_FILTER);
	//
...
static int vencoder_get_sps_pps(int iid)
{
...
static int vencoder_reconfigure(int iid, ga_ioctl_reconfigure_t* reconf)
{
	if(vencoder_initialized == 0)
	{
	...
	if(reconf->bitrateKbps > 0 || reconf->framerate_n > 0)
	{
