...
int mfx_encode(int cid)
{
	...
	while(sts >= MFX_ERR_NONE || sts == MFX_ERR_MORE_DATA)
	{
		...
		while(ptr != NULL)
		{
			...
			for(nextptr = ptr + 3; nextptr < _mfxbs[cid].Data + _mfxbs[cid].DataOffset + _mfxbs[cid].DataLength - 4; nextptr++)
			{
...
unsigned long long load_frame_count() { return framecount; }
...
unsigned long long write_frame_bytes() { return byteswritten; }
