...
#define PFX "video encoder: "
	switch(sts)
	{
		...
		case MFX_ERR_NULL_PTR:
			ret = -1;
			ga_error(PFX "MFX_ERR_NULL_PTR\n");
			break;
		case MFX_ERR_UNSUPPORTED:
			ret = -1;
			ga_error(PFX "MFX_ERR_UNSUPPORTED\n");
			break;
		case MFX_ERR_MEMORY_ALLOC:
			ret = -1;
			ga_error(PFX "MFX_ERR_MEMORY_ALLOC\n");
			break;
		case MFX_ERR_NOT_ENOUGH_BUFFER:
			ret = -1;
			ga_error(PFX "MFX_ERR_NOT_ENOUGH_BUFFER\n");
			break;
		case MFX_ERR_INVALID_HANDLE:
			ret = -1;
			ga_error(PFX "MFX_ERR_INVALID_HANDLE\n");
			break;
		case MFX_ERR_LOCK_MEMORY:
			ret = -1;
			ga_error(PFX "MFX_ERR_LOCK_MEMORY\n");
			break;
		case MFX_ERR_NOT_INITIALIZED:
			ret = -1;
			ga_error(PFX "MFX_ERR_NOT_INITIALIZED\n");
			break;
		case MFX_ERR_NOT_FOUND:
			ret = -1;
			ga_error(PFX "MFX_ERR_NOT_FOUND\n");
			break;
		case MFX_ERR_MORE_DATA:
			ret = -1;
			ga_error(PFX "MFX_ERR_MORE_DATA\n");
			break;
		case MFX_ERR_MORE_SURFACE:
			ret = -1;
			ga_error(PFX "MFX_ERR_MORE_SURFACE\n");
			break;
		case MFX_ERR_ABORTED:
			ret = -1;
			ga_error(PFX "MFX_ERR_ABORTED\n");
			break;
		case MFX_ERR_DEVICE_LOST:
			ret = -1;
			ga_error(PFX "MFX_ERR_DEVICE_LOST\n");
			break;
		case MFX_ERR_INCOMPATIBLE_VIDEO_PARAM:
			ret = -1;
			ga_error(PFX "MFX_ERR_INCOMPATIBLE_VIDEO_PARAM\n");
			break;
		case MFX_ERR_INVALID_VIDEO_PARAM:
			ret = -1;
			ga_error(PFX "MFX_ERR_INVALID_VIDEO_PARAM\n");
			break;
		case MFX_ERR_UNDEFINED_BEHAVIOR:
			ret = -1;
			ga_error(PFX "MFX_ERR_UNDEFINED_BEHAVIOR\n");
			break;
		case MFX_ERR_DEVICE_FAILED:
			ret = -1;
			ga_error(PFX "MFX_ERR_DEVICE_FAILED\n");
			break;
		case MFX_ERR_MORE_BITSTREAM:
			ret = -1;
			ga_error(PFX "MFX_ERR_MORE_BITSTREAM\n");
			break;
		case MFX_ERR_INCOMPATIBLE_AUDIO_PARAM:
			ret = -1;
			ga_error(PFX "MFX_ERR_INCOMPATIBLE_AUDIO_PARAM\n");
			break;
		case MFX_ERR_INVALID_AUDIO_PARAM:
			ret = -1;
			ga_error(PFX "MFX_ERR_INVALID_AUDIO_PARAM\n");
			break;
		/* warnings >0 */
		case MFX_WRN_IN_EXECUTION:
			ga_error(PFX "MFX_WRN_IN_EXECUTION\n");
			break;
		case MFX_WRN_DEVICE_BUSY:
			ga_error(PFX "MFX_WRN_DEVICE_BUSY\n");
			break;
		case MFX_WRN_VIDEO_PARAM_CHANGED:
			ga_error(PFX "MFX_WRN_VIDEO_PARAM_CHANGED\n");
			break;
		case MFX_WRN_PARTIAL_ACCELERATION:
			ga_error(PFX "MFX_WRN_PARTIAL_ACCELERATION\n");
			break;
		case MFX_WRN_INCOMPATIBLE_VIDEO_PARAM:
			ga_error(PFX "MFX_WRN_INCOMPATIBLE_VIDEO_PARAM\n");
			break;
		case MFX_WRN_VALUE_NOT_CHANGED:
			ga_error(PFX "MFX_WRN_VALUE_NOT_CHANGED\n");
			break;
		case MFX_WRN_OUT_OF_RANGE:
			ga_error(PFX "MFX_WRN_OUT_OF_RANGE\n");
			break;
		case MFX_WRN_FILTER_SKIPPED:
			ga_error(PFX "MFX_WRN_FILTER_SKIPPED\n");
			break;
		case MFX_WRN_INCOMPATIBLE_AUDIO_PARAM:
			ga_error(PFX "WRN_INCOMPATIBLE_AUDIO_PARAM\n");
			break;
...
mfxStatus mfx_init_vpp(mfxSession s, int width, int height, int fps, int RGBmode)
{
	mfxVideoParam init_param;
	//
	memset(&init_param, 0, sizeof(init_param));
	init_param.AsyncDepth = 1;
	init_param.IOPattern	 = MFX_IOPATTERN_IN_SYSTEM_MEMORY | MFX_IOPATTERN_OUT_SYSTEM_MEMORY;
	//
...
mfxStatus mfx_init_encoder(mfxSession s, int width, int height, int fps, int bitrateKbps)
{
	...
	init_param.IOPattern	 = MFX_IOPATTERN_IN_SYSTEM_MEMORY;
	//
	...
	ga_error("video encoder: %dx%d (%dx%d) at %.1ffps, gop=%d, usage=%d, target=%dKbps\n",
				width,
				height,
				init_param.mfx.FrameInfo.Width,
				init_param.mfx.FrameInfo.Height,
...
int mfx_init_internal(int cid, int width, int height, int fps, int bitrateKbps, int RGBmode)
{
	mfxStatus sts;
	//
	...
	//
	if(mfx_vpp_pool(
		  _session[cid], &_vppparam[cid], &_vppresponse[cid][0], &_vpppool[cid][0], &_vppresponse[cid][1], &_vpppool[cid][1]) !=
		MFX_ERR_NONE)
	{
