...
static int mfx_deinit(void* arg)
{
	int iid;
...
static void* mfx_threadproc(void* arg)
{
	...
	//
	while(mfx_started != 0 && encoder_running() > 0 && (sts >= MFX_ERR_NONE || sts == MFX_ERR_MORE_DATA))
	{
		//
		...
		if(RGBmode == 0)
		{
			...
			// copy the frame
			if(svppin->Data.Pitch < frame->linesize[0] || svppin->Data.Pitch / 2 < frame->linesize[1] ||
		...
		else
		{
			...
			{
				ga_error(
				  "video encoder: RGB/ERROR - surface->Pitch (%d) < frame->linesize (%d)\n", svppin->Data.Pitch, frame->linesize[0]);
				exit(-1);
				break;
			}
			// Copy
#define MSDK_MIN(a, b) ((a) < (b) ? (a) : (b))
			for(dst = MSDK_MIN(MSDK_MIN(svppin->Data.R, svppin->Data.G), svppin->Data.B), i = 0; i < outputH; i++)
			{
...
#undef MSDK_MIN
		...
		if((ptr = ga_find_startcode(_mfxbs[cid].Data + _mfxbs[cid].DataOffset,
											 ...
											 &offset)) != _mfxbs[cid].Data + _mfxbs[cid].DataOffset)
		{
		...
		while(ptr != NULL)
		{
			...
			if((*(ptr + offset) & 0x1f) > 5)
			{
				...
				pkt.data = ptr;
				pkt.size =
				  nextptr != NULL ? (nextptr - ptr) : (_mfxbs[cid].Data + _mfxbs[cid].DataOffset + _mfxbs[cid].DataLength - ptr);
...
static void mfx_dump_buffer(const char* prefix, unsigned char* buf, int buflen)
{
	char output[2048];
	int size = 0, wlen;
	// head
...
static int mfx_get_sps_pps(int iid)
{
	void* ret = NULL;
...
static int mfx_ioctl(int command, int argsize, void* arg)
{
	int ret								 = 0;
