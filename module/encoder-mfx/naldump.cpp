...
enum nal_state { NS_IDLE = 0, NS_FOUND_0, NS_FOUND_00, NS_FOUND_000, NS_FOUND_001, NS_FOUND_0001 };
...
void output_nal(int type, long long offset, long long size)
{
	//
	nal_count++;
	nal_totalsize += size;
...
void output_statistic()
{
	...
	printf("# nal  count    %%    nal  count    %%    nal  count    %%    nal  count    %%\n");
	printf("# ---- -------- ---- ---- -------- ---- ---- -------- ---- ---- -------- ----\n");
	for(i = 0; i < 8; i++)
	{
		printf("# ");
		...
		{
			int idx = i + j * 8;
			printf("%02d   %-8lld %-4.1f ", idx, nal_type[idx], 100.0 * nal_type[idx] / nal_count);
		}
		printf("\n");
	}
	return;
