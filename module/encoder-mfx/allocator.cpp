...
static int bafa_error(const char* fmt, ...)
{
#if 1
	va_list ap;
	va_start(ap, fmt);
...
mfxStatus ba_unlock(mfxHDL pthis, mfxMemId mid) { return MFX_ERR_NONE; }
...
mfxStatus fa_alloc(mfxHDL pthis, mfxFrameAllocRequest* request, mfxFrameAllocResponse* response)
{
	...
	switch(request->Info.FourCC)
	{
		...
		case MFX_FOURCC_NV12:
			nbytes = allocwidth * allocheight * 3 / 2;
			break;
		case MFX_FOURCC_RGB3:
			nbytes = allocwidth * allocheight * 3;
			break;
		case MFX_FOURCC_RGB4:
			nbytes = allocwidth * allocheight * 4;
			break;
		case MFX_FOURCC_YUY2:
			nbytes = allocwidth * allocheight * 2;
			break;
...
mfxStatus fa_gethdl(mfxHDL pthis, mfxMemId mid, mfxHDL* handle) { return MFX_ERR_UNSUPPORTED; }
