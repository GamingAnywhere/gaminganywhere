...
int ga_xwin_init(const char* displayname, gaImage* gaimg)
{
	...
	ga_error("X-Window-init: dimension: %dx%dx%d @ %d/%d\n", width, height, depth, screenNumber, XScreenCount(display));
	//
	if((image = XShmCreateImage(
			display, XDefaultVisual(display, screenNumber), depth, ZPixmap, NULL, &__xshminfo, width, height)) == NULL)
	{
		ga_error("XShmCreateImage failed.\n");
		goto xcap_init_error;
	}
	//
	if((__xshminfo.shmid = shmget(IPC_PRIVATE, image->bytes_per_line * image->height, IPC_CREAT | 0777)) < 0)
	{
		perror("shmget");
		goto xcap_init_error;
	}
...
void ga_xwin_imageinfo(XImage* image)
{
	...
	ga_error(
	  "ga-imageinfo: --> mask red=0x%08lx green=0x%08lx blue=0x%08lx\n", image->red_mask, image->green_mask, image->blue_mask);
	return;
}
