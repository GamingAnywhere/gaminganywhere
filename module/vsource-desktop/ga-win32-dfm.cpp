...
// DFM structures
static const char MINIPORT_REGISTRY_PATH[] = "SYSTEM\\CurrentControlSet\\Hardware Profiles\\"
...
static int registrykey_open(HKEY root, const char* entry, bool createIfNotExist, HKEY* openedKey)
{
...
static int dfm_extractDeviceInfo(char* driverName)
{
	BOOL result;
	//
...
static int dfm_openDeviceRegKey(char* miniportName)
{
	int pos;
...
static int dfm_setAttachToDesktop(bool attach)
{
...
static int dfm_commitDisplayChanges(DEVMODE* pdm)
{
...
static int dfm_unload()
{
	...
	if(m_isDriverAttached)
	{
		...
		DEVMODE* pdm = NULL;
		if(osVersion.dwMajorVersion == 5 && osVersion.dwMinorVersion == 0 /* Windows 2000*/)
		{
			pdm = &m_deviceMode;
		}
...
int ga_win32_DFM_init(struct gaImage* image)
{
	...
	//
	ga_win32_fill_bitmap_info(&bmpInfo, GetSystemMetrics(SM_CXSCREEN), GetSystemMetrics(SM_CYSCREEN), BITSPERPIXEL);
