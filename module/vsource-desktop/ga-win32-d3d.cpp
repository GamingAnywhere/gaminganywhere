...
static void MakeBitmapInfo(BITMAPINFO* pInfo, int w, int h, int bitsPerPixel)
{
...
int ga_win32_D3D_init(struct gaImage* image)
{
	...
	if(FAILED(g_pd3dDevice->CreateOffscreenPlainSurface(
		  ddm.Width, ddm.Height, D3DFMT_A8R8G8B8, D3DPOOL_SYSTEMMEM /*D3DPOOL_SCRATCH*/, &g_pSurface, NULL)))
	{
...
int ga_win32_D3D_capture(char* buf, int buflen, struct gaRect* grect)
{
	...
	// lock
	if(FAILED(g_pSurface->LockRect(&lockedRect, &screenRect, D3DLOCK_NO_DIRTY_UPDATE | D3DLOCK_NOSYSLOCK | D3DLOCK_READONLY)))
	{
