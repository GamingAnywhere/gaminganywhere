...
enum AVStreamParseType {
	...
	AVSTREAM_PARSE_FULL_RAW =
	  MKTAG(0, 'R', 'A', 'W'), /**< full parsing and repack with timestamp and position generation by parser for raw
...
#define AVINDEX_KEYFRAME 0x0001
	int flags : 2;
	int size : 30; // Yeah, trying to keep the size of this small to reduce memory requirements (it is 24 vs. 32 bytes due to
...
#define AVFMT_FLAG_NOPARSE \
	0x0020 ///< Do not use AVParsers, you also must set AVFMT_FLAG_NOFILLIN as the fillin code works on frames and no parsing -> no
			 ///< frames. Also seeking to frames can not work if parsing to find frame boundaries has been disabled
...
#define AVFMT_FLAG_PRIV_OPT \
	0x20000 ///< Enable use of private options by delaying codec open (this could be made default once all code is converted)
...
attribute_deprecated void av_pkt_dump_log(void* avcl, int level, AVPacket* pkt, int dump_payload);
#endif
