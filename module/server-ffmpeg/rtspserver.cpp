...
static int rtsp_write(RTSPContext* ctx, const void* buf, size_t count) { return write(ctx->fd, buf, count); }

static int rtsp_printf(RTSPContext* ctx, const char* fmt, ...)
{
	va_list ap;
	char buf[8192];
	int buflen;
...
#ifdef HOLE_PUNCHING
static int rtp_open_internal(unsigned short* port)
{
#ifdef WIN32
	SOCKET s;
#else
...
int rtp_close_ports(RTSPContext* ctx, int streamid)
{
	streamid *= 2;
	if(ctx->rtpSocket[streamid] != 0)
		close(ctx->rtpSocket[streamid]);
...
#endif
		sendto(
		  ctx->rtpSocket[streamid * 2], (const char*)&buf[i + 4], pktlen, 0, (struct sockaddr*)&sin, sizeof(struct sockaddr_in));
		i += (4 + pktlen);
	}
	return i;
}
...
static int rtsp_read_internal(RTSPContext* ctx)
{
	int rlen;
	if((rlen = read(ctx->fd, ctx->rbuffer + ctx->rbuftail, ctx->rbufsize - ctx->rbuftail)) <= 0)
	{
		return -1;
	}
	ctx->rbuftail += rlen;
...
static int rtsp_read_text(RTSPContext* ctx, char* buf, size_t count)
{
	int i;
	size_t textlen;
again:
...
static int rtsp_read_binary(RTSPContext* ctx, char* buf, size_t count)
{
	int reqlength;
...
static int rtsp_getnext(RTSPContext* ctx, char* buf, size_t count)
{
...
static int per_client_init(RTSPContext* ctx)
{
	int i;
	AVOutputFormat* fmt;
	//
	...
	{
		if((ctx->sdp_vstream[i] = ga_avformat_new_stream(ctx->sdp_fmtctx, i, rtspconf->video_encoder_codec)) == NULL)
		{
			//
			ga_error("cannot create new video stream (%d:%d)\n", i, rtspconf->video_encoder_codec->id);
			return -1;
		}
		if((ctx->sdp_vencoder[i] = ga_avcodec_vencoder_init(ctx->sdp_vstream[i]->codec,
																			 rtspconf->video_encoder_codec,
																			 video_source_out_width(i),
																			 video_source_out_height(i),
																			 rtspconf->video_fps,
																			 rtspconf->vso)) == NULL)
		{
			//
	...
	// audio stream
#ifdef ENABLE_AUDIO
	if((ctx->sdp_astream = ga_avformat_new_stream(ctx->sdp_fmtctx, video_source_channels(), rtspconf->audio_encoder_codec)) ==
		NULL)
	{
		ga_error("cannot create new audio stream (%d)\n", rtspconf->audio_encoder_codec->id);
		return -1;
	}
	if((ctx->sdp_aencoder = ga_avcodec_aencoder_init(ctx->sdp_astream->codec,
...
static void close_av(AVFormatContext* fctx, AVStream* st, AVCodecContext* cctx, enum RTSPLowerTransport transport)
{
	unsigned i;
	//
	if(cctx)
...
static void per_client_deinit(RTSPContext* ctx)
{
	int i;
...
static int prepare_sdp_description(RTSPContext* ctx, char* buf, int bufsize)
{
	buf[0] = '\0';
...
static void rtsp_cmd_describe(RTSPContext* ctx, const char* url)
{
...
static int rtp_new_av_stream(RTSPContext* ctx, struct sockaddr_in* sin, int streamid, enum AVCodecID codecid)
{
	...
	if(codecid != rtspconf->video_encoder_codec->id && codecid != rtspconf->audio_encoder_codec->id)
	{
	...
	if(ctx->mtu > 0)
	{
		...
		{
			fmtctx->packet_size = ctx->mtu < fmtctx->packet_size ? ctx->mtu : fmtctx->packet_size;
		}
		else
		{
	...
	if(ctx->lower_transport[streamid] == RTSP_LOWER_TRANSPORT_UDP)
	{
		snprintf(fmtctx->filename, sizeof(fmtctx->filename), "rtp://%s:%d", inet_ntoa(sin->sin_addr), ntohs(sin->sin_port));
	...
	//
	if((stream = ga_avformat_new_stream(fmtctx,
													0,
													codecid == rtspconf->video_encoder_codec->id ? rtspconf->video_encoder_codec
	...
	{
		encoder = ga_avcodec_vencoder_init(stream->codec,
													  rtspconf->video_encoder_codec,
	...
	{
		encoder = ga_avcodec_aencoder_init(stream->codec,
													  rtspconf->audio_encoder_codec,
													  rtspconf->audio_bitrate,
													  rtspconf->audio_samplerate,
													  rtspconf->audio_channels,
...
static void rtsp_cmd_setup(RTSPContext* ctx, const char* url, RTSPMessageHeader* h)
{
	int i;
	RTSPTransportField* th;
	...
	//
	if(ctx->state != SERVER_STATE_IDLE && ctx->state != SERVER_STATE_READY)
	{
	...
	if(ctx->session_id == NULL || strcmp(ctx->session_id, h->session_id) != 0)
	{
	...
	if(rtp_new_av_stream(ctx,
								&destaddr,
								streamid,
								streamid == video_source_channels() /*rtspconf->audio_id*/ ? rtspconf->audio_encoder_codec->id
																											  : rtspconf->video_encoder_codec->id) < 0)
	{
...
static void rtsp_cmd_play(RTSPContext* ctx, const char* url, RTSPMessageHeader* h)
{
	char path[4096];
	//
	av_url_split(NULL, 0, NULL, 0, NULL, 0, NULL, path, sizeof(path), url);
	...
	//
	if(ctx->state != SERVER_STATE_READY && ctx->state != SERVER_STATE_PAUSE)
	{
		rtsp_reply_error(ctx, RTSP_STATUS_STATE);
		return;
	}
...
static void rtsp_cmd_pause(RTSPContext* ctx, const char* url, RTSPMessageHeader* h)
{
	char path[4096];
	//
	av_url_split(NULL, 0, NULL, 0, NULL, 0, NULL, path, sizeof(path), url);
...
static void rtsp_cmd_teardown(RTSPContext* ctx, const char* url, RTSPMessageHeader* h, int bruteforce)
{
	char path[4096];
	//
	av_url_split(NULL, 0, NULL, 0, NULL, 0, NULL, path, sizeof(path), url);
...
void* rtspserver(void* arg)
{
	...
	//
	ga_error("[tid %ld] client connected from %s:%d\n", ga_gettid(), inet_ntoa(sin.sin_addr), htons(sin.sin_port));
	//
	ctx.fd = s;
	//
	do
