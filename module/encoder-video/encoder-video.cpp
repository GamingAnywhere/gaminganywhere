...
static int vencoder_init(void* arg)
{
	...
	for(iid = 0; iid < video_source_channels(); iid++)
	{
		...
		vencoder[iid] =
		  ga_avcodec_vencoder_init(NULL, rtspconf->video_encoder_codec, outputW, outputH, rtspconf->video_fps, rtspconf->vso);
		if(vencoder[iid] == NULL)
			goto init_failed;
#ifdef STANDALONE_SDP
		...
		switch(rtspconf->video_encoder_codec->id)
		{
			...
			case AV_CODEC_ID_MPEG4:
				...
				avc =
				  ga_avcodec_vencoder_init(avc, rtspconf->video_encoder_codec, outputW, outputH, rtspconf->video_fps, rtspconf->vso);
				if(avc == NULL)
					goto init_failed;
...
static int vencoder_reconfigure(int iid)
{
	...
	if(reconf->id >= 0)
	{
		...
		ga_avcodec_vencoder_init(
		  vencoder[iid], rtspconf->video_encoder_codec, outputW, outputH, rtspconf->video_fps, rtspconf->vso);
...
static void* vencoder_threadproc(void* arg)
{
	...
	avpicture_fill((AVPicture*)pic_in, pic_in_buf, AV_PIX_FMT_YUV420P, outputW, outputH);
	...
	while(vencoder_started != 0 && encoder_running() > 0)
	{
		...
		if(pic_in->linesize[0] == frame->linesize[0] && pic_in->linesize[1] == frame->linesize[1] &&
		...
		if(got_packet)
		{
			...
			// send the packet
			if(encoder_send_packet("video-encoder", iid /*rtspconf->video_id*/, &pkt, pkt.pts, &tv) < 0)
			{
				goto video_quit;
			}
...
static int h264or5_get_vparam(int type, int channelId, unsigned char* data, int datalen)
{
	...
	if(sps != NULL && pps != NULL)
	{
		...
		{
			ga_error(
			  "video encoder: h.264/found sps@%d(%d); pps@%d(%d)\n", sps - data, _spslen[channelId], pps - data, _ppslen[channelId]);
		}
		//
		ret = 0;
	}
	...
	switch(command)
	{
		...
		case GA_IOCTL_GETVPS:
			...
			if(ve->codec_id == AV_CODEC_ID_H264 && command == GA_IOCTL_GETVPS)
				return GA_IOCTL_ERR_NOTSUPPORTED;
			if(h264or5_get_vparam(ve->codec_id == AV_CODEC_ID_H264 ? 264 : 265, buf->id, ve->extradata, ve->extradata_size) < 0)
			{
				return GA_IOCTL_ERR_NOTFOUND;
			}
