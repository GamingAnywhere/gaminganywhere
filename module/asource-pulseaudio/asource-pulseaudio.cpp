...
static int asource_init(void* arg)
{
	int error;
	...
	pa_ctx			  = pa_simple_new(NULL,
									"gaminganywhere-asource-pulseaudio",
									PA_STREAM_RECORD,
									dev,
									"gaminganywhere-record-stream",
									&pa_spec,
									NULL,
									NULL,
									&error);
...
static void* asource_threadproc(void* arg)
{
	int error;
	int framesize;
...
static int asource_deinit(void* arg)
{
	if(pa_ctx != NULL)
		pa_simple_free(pa_ctx);
	pa_ctx				  = NULL;
...
static int asource_start(void* arg)
{
...
static int asource_stop(void* arg)
{
	if(asource_started == 0)
		return 0;
	asource_started = 0;
