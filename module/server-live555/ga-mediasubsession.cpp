...
GAMediaSubsession ::GAMediaSubsession(UsageEnvironment& env,
												  int cid,
												  const char* mimetype,
...
GAMediaSubsession* GAMediaSubsession ::createNew(UsageEnvironment& env,
																 int cid,
																 const char* mimetype,
																 portNumBits initialPortNum,
																 Boolean multiplexRTCPWithRTP)
...
FramedSource* GAMediaSubsession ::createNewStreamSource(unsigned clientSessionId, unsigned& estBitrate)
{
...
RTPSink* GAMediaSubsession ::createNewRTPSink(Groupsock* rtpGroupsock,
															 ...
															 FramedSource* inputSource)
{
	...
	{
		result = QoSAC3AudioRTPSink ::createNew(envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, rtspconf->audio_samplerate);
	}
	else if(strcmp(mimetype, "audio/OPUS") == 0)
	{
		result = QoSSimpleRTPSink ::createNew(envir(),
														  rtpGroupsock,
	...
	else if(strcmp(mimetype, "audio/VORBIS") == 0)
	{
		...
		ga_error("GAMediaSubsession: %s NOT IMPLEMENTED\n", mimetype);
		exit(-1);
		result = QoSVorbisAudioRTPSink ::createNew(envir(),
																 rtpGroupsock,
																 rtpPayloadTypeIfDynamic,
																 rtspconf->audio_samplerate,
	...
	else if(strcmp(mimetype, "video/THEORA") == 0)
	{
		...
		ga_error("GAMediaSubsession: %s NOT IMPLEMENTED\n", mimetype);
		exit(-1);
		result = QoSTheoraVideoRTPSink ::createNew(envir(),
																 rtpGroupsock,
																 rtpPayloadTypeIfDynamic,
																 identificationHeader,
																 identificationHeaderSize,
	...
	else if(strcmp(mimetype, "video/H264") == 0)
	{
		...
		ga_error("GAMediaSubsession: %s SPS=%p(%d); PPS=%p(%d); profile_level_id=%x\n",
					...
					PPSSize,
					profile_level_id);
		result = QoSH264VideoRTPSink::createNew(
		  envir(), rtpGroupsock, rtpPayloadTypeIfDynamic, SPS, SPSSize, PPS, PPSSize /*, profile_level_id*/);
	}
