...
static int aencoder_deinit(void* arg)
{
	...
	if(swrctx)
		swr_free(&swrctx);
	if(encoder)
		ga_avcodec_close(encoder);
	if(encoder_sdp)
		ga_avcodec_close(encoder_sdp);
	//
	swrctx		= NULL;
	convbuf		= NULL;
	encoder		= NULL;
...
static int aencoder_init(void* arg)
{
	...
	// alloc encoder
	encoder = ga_avcodec_aencoder_init(NULL,
	...
	source_size = av_samples_get_buffer_size(
	  NULL, rtspconf->audio_channels, encoder->frame_size, rtspconf->audio_device_format, 1 /*no-alignment*/);
	encoder_size =
	  av_samples_get_buffer_size(dstlines, encoder->channels, encoder->frame_size, encoder->sample_fmt, 1 /*no-alignment*/);
#if 1
	do
	{
		int i = 0;
	...
	while(aencoder_started != 0 && encoder_running() > 0)
	{
		...
		while(nsamples >= encoder->frame_size)
		{
			...
			if(swrctx != NULL)
			{
				...
				srcplanes[0] = srcbuf;
				srcplanes[1] = NULL;
				swr_convert(swrctx, dstplanes, encoder->frame_size, srcplanes, encoder->frame_size);
				srcbuf  = convbuf;
				srcsize = encoder_size;
			}
			//
			if(avcodec_fill_audio_frame(snd_in,
												 encoder->channels,
												 encoder->sample_fmt,
												 srcbuf /*samples+offset*/,
												 srcsize /*encoder_size*/,
