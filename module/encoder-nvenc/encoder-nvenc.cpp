						...
						: (encodeConfig.presetGUID == NV_ENC_PRESET_HQ_GUID)
							 ...
							 : (encodeConfig.presetGUID == NV_ENC_PRESET_HP_GUID)
								  ...
								  : (encodeConfig.presetGUID == NV_ENC_PRESET_LOSSLESS_HP_GUID)
										? "LOSSLESS_HP"
										: (encodeConfig.presetGUID == NV_ENC_PRESET_LOW_LATENCY_DEFAULT_GUID) ? "LOW_LATENCY_DEFAULT"
																																	 : "DEFAULT");
	ga_error("--------------------------------------------------------------------------------------\n\n");
...
int CNvEncoderLowLatency::ThreadProc()
{
	...
	for(int frm = encodeConfig.startFrameIdx; frm <= encodeConfig.endFrameIdx && vencoder_started != 0; frm++) // need to be fixed!
	{
		...
		nvStatus = PreProcessInput(pEncodeBuffer,
											stEncodeFrame.yuv,
											stEncodeFrame.width,
											stEncodeFrame.height,
											m_pNvHWEncoder->m_uCurWidth,
											m_pNvHWEncoder->m_uCurHeight,
											m_pNvHWEncoder->m_uMaxWidth,
											m_pNvHWEncoder->m_uMaxHeight);
		if(nvStatus != NV_ENC_SUCCESS)
		{
		...
		// send the packet
		if(encoder_send_packet("video-encoder", 0 /*iid*/, &pkt, pkt.pts, NULL) < 0)
		{
			goto exit;
		}
	}
 ...
 */
static void nvenc_dump_buffer(const char* prefix, unsigned char* buf, int buflen)
{
	char output[2048];
	int size = 0, wlen;
	// head
...
int CNvEncoderLowLatency::GetSPSPPS()
{
	...
	while(tmpHeader[i_sps] != 0x00 || tmpHeader[i_sps + 1] != 0x00 || tmpHeader[i_sps + 2] != 0x00 || tmpHeader[i_sps + 3] != 0x01)
	{
		i_sps += 1;
		if(i_sps >= outSize)
		{
 ...
 */
static int nvenc_deinit(void* arg)
{
#ifdef SAVEENC
	if(encodeConfig.fOutput)
	{
 ...
 *	Encoder initialize
 */
static int nvenc_init(void* arg)
{
	if(vencoder_initialized != 0)
		return 0;
 ...
 */
static int nvenc_reconfigure(int kbitrate, int buffsize)
{
	int sts;
 ...
 *	Encoder thread proc
 */
static void* nvenc_threadproc(void* arg)
{
 ...
 *	Encoder start
 */
static int nvenc_start(void* arg)
{
 ...
 *	Encoder stop
 */
static int nvenc_stop(void* arg)
{
	void* ignore;
	if(vencoder_started == 0)
		return 0;
 ...
 *	Encoder IO Contrl
 */
static int nvenc_ioctl(int command, int argsize, void* arg)
{
	int ret								 = 0;
