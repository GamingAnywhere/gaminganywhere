...
NVENCSTATUS CNvEncoderLowLatency::PreProcessInput(EncodeBuffer* pEncodeBuffer,
																  unsigned char* yuv[3],
																  uint32_t srcWidth,
																  uint32_t srcHeight,
																  uint32_t dstWidth,
																  ...
																  uint32_t maxHeight)
{
	...
	if(bScaling)
	{
		ConvertYUVToNV12(pEncodeBuffer->stInputBfr.pNV12TempdevPtr,
							  pEncodeBuffer->stInputBfr.uNV12TempStride,
							  yuv,
							  srcWidth,
							  srcHeight,
							  maxWidth,
							  maxHeight);
		ScaleNV12Image(pEncodeBuffer->stInputBfr.pNV12TempdevPtr,
	...
	{
		ConvertYUVToNV12(pEncodeBuffer->stInputBfr.pNV12devPtr,
							  pEncodeBuffer->stInputBfr.uNV12Stride,
							  yuv,
							  dstWidth,
							  dstHeight,
...
NVENCSTATUS CNvEncoderLowLatency::ConvertYUVToNV12(CUdeviceptr dNV12devPtr,
																	int dstPitch,
																	unsigned char* yuv[3],
																	int width,
																	int height,
...
#undef BLOCK_X
	...
	void* args[8]				= {
		&m_ChromaDevPtr[0], &m_ChromaDevPtr[1], &dNV12Chroma, &chromaWidth, &chromaHeight, &chromaWidth, &chromaWidth, &dstPitch};

	__cu(cuLaunchKernel(m_cuInterleaveUVFunction, grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL));
...
CUresult CNvEncoderLowLatency::ScaleNV12Image(CUdeviceptr dInput,
															 CUdeviceptr dOutput,
															 int srcWidth,
															 int srcPitch,
															 int srcHeight,
...

{
	...
	result = cuLaunchKernel(m_cuScaleNV12Function, grid.x, grid.y, grid.z, block.x, block.y, block.z, 0, NULL, args, NULL);
	if(result != CUDA_SUCCESS)
	{
