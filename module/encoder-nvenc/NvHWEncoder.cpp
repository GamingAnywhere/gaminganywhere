...
NVENCSTATUS CNvHWEncoder::NvEncGetEncodeProfileGUIDs(GUID encodeGUID,
																	  GUID* profileGUIDs,
...
NVENCSTATUS CNvHWEncoder::NvEncGetInputFormats(GUID encodeGUID,
...
NVENCSTATUS CNvHWEncoder::NvEncGetEncodePresetGUIDs(GUID encodeGUID,
																	 GUID* presetGUIDs,
																	 uint32_t guidArraySize,
...
NVENCSTATUS CNvHWEncoder::NvRunMotionEstimationOnly(EncodeBuffer* pEncodeBuffer[2], MEOnlyConfig* pMEOnly)
{
	...
	if(m_fOutput)
	{
		...
		fprintf(m_fOutput,
				  "Motion Vectors for input frame = %d, reference frame = %d\n",
				  pMEOnly->inputFrameIndex,
				  pMEOnly->referenceFrameIndex);
		NV_ENC_H264_MV_DATA* outputMV = (NV_ENC_H264_MV_DATA*)stMEOnlyParams.outputMV;
		for(unsigned int i = 0; i < numMBs; i++)
		{
			fprintf(m_fOutput,
					  "block = %d, mb_type = %d, partitionType = %d, MV[0].x = %d, MV[0].y = %d, MV[1].x = %d, MV[1].y = %d, MV[2].x "
					  "= %d, MV[2].y = %d, MV[3].x = %d, MV[3].y = %d, cost=%d ",
					  i,
					  outputMV[i].mb_type,
					  outputMV[i].partitionType,
					  outputMV[i].MV[0].mvx,
					  outputMV[i].MV[0].mvy,
					  outputMV[i].MV[1].mvx,
					  outputMV[i].MV[1].mvy,
...
NVENCSTATUS CNvHWEncoder::NvEncRegisterResource(NV_ENC_INPUT_RESOURCE_TYPE resourceType,
...
NVENCSTATUS CNvHWEncoder::NvEncReconfigureEncoder(const NvEncPictureCommand* pEncPicCommand)
{
	...
	if(pEncPicCommand->bBitrateChangePending || pEncPicCommand->bResolutionChangePending)
	{
		...
		if(pEncPicCommand->bBitrateChangePending)
		{
			...
			m_stEncodeConfig.rcParams.vbvBufferSize =
			  pEncPicCommand->newVBVSize != 0
				 ? pEncPicCommand->newVBVSize
				 : (pEncPicCommand->newBitrate * m_stCreateEncodeParams.frameRateDen) / m_stCreateEncodeParams.frameRateNum;
...
NVENCSTATUS CNvHWEncoder::ValidatePresetGUID(GUID inputPresetGuid, GUID inputCodecGuid)
{
	...
	nvStatus =
	  m_pEncodeAPI->nvEncGetEncodePresetGUIDs(m_hEncoder, inputCodecGuid, presetGUIDArray, presetGUIDCount, &presetGUIDArraySize);
	if(nvStatus != NV_ENC_SUCCESS)
	{
		assert(0);
...
NVENCSTATUS CNvHWEncoder::CreateEncoder(const EncodeConfig* pEncCfg)
{
	...
	nvStatus = m_pEncodeAPI->nvEncGetEncodePresetConfig(
	  m_hEncoder, m_stCreateEncodeParams.encodeGUID, m_stCreateEncodeParams.presetGUID, &stPresetCfg);
	if(nvStatus != NV_ENC_SUCCESS)
	{
...
NVENCSTATUS CNvHWEncoder::NvEncEncodeFrame(EncodeBuffer* pEncodeBuffer,
