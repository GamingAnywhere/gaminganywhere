...
CUtexref CNvEncoderLowLatency::InitTexture(CUmodule hModule,
														 const char* name,
														 CUarray_format fmt,
...
NVENCSTATUS CNvEncoderLowLatency::AllocateIOBuffers(uint32_t uInputWidth, uint32_t uInputHeight)
{
	...
	for(uint32_t i = 0; i < m_uEncodeBufferCount; i++)
	{
		__cu(cuMemAllocPitch(&m_stEncodeBuffer[i].stInputBfr.pNV12devPtr,
									...
									uInputHeight * 3 / 2,
									16));
		__cu(cuMemAllocPitch(&m_stEncodeBuffer[i].stInputBfr.pNV12TempdevPtr,
									(size_t*)&m_stEncodeBuffer[i].stInputBfr.uNV12TempStride,
									uInputWidth,
									uInputHeight * 3 / 2,
									16));
...
void PrintHelp()
{
	printf("Usage : NvEncoderLowlatency \n"
			 ...
			 "                             4: Invalidate Refrence frame <param0 = ref frame 0> <param0 = ref frame 1> ..<param15 = "
			 "ref frame 15>\n"
