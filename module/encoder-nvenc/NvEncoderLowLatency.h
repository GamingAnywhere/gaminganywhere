...
class CNvEncoderLowLatency
{
  ...
  protected:
	...
	CUtexref InitTexture(CUmodule hModule,
								const char* name,
								CUarray_format fmt,
								int NumPackedComponents = 1,
	...
	NVENCSTATUS PreProcessInput(EncodeBuffer* pEncodeBuffer,
										 unsigned char* yuv[3],
										 uint32_t srcWidth,
										 uint32_t srcHeight,
										 uint32_t dstWidth,
										 ...
										 uint32_t maxWidth,
										 uint32_t maxHeight);
	NVENCSTATUS ConvertYUVToNV12(CUdeviceptr dNV12devPtr,
										  int dstPitch,
										  unsigned char* yuv[3],
										  int width,
										  int height,
	...
	CUresult ScaleNV12Image(CUdeviceptr dInput,
									CUdeviceptr dOutput,
									int srcWidth,
									int srcPitch,
									int srcHeight,
