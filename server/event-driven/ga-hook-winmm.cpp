...
static enum AVSampleFormat WINMM2SWR_format(WAVEFORMATEX* w)
{
...
static int64_t WINMM2SWR_chlayout(int channels)
{
	if(channels == 1)
...
static int winmm_create_swrctx(WAVEFORMATEX* w)
{
	...
	//
	ga_error(
	  "hook_winmm: create swr context - format[%x] freq[%d] channels[%d]\n", w->wFormatTag, w->nSamplesPerSec, w->nChannels);
	//
	...
	samples					  = av_rescale_rnd(CA_MAX_SAMPLES, rtspconf->audio_samplerate, w->nSamplesPerSec, AV_ROUND_UP);
	bufreq =
...
MMRESULT
hook_waveOutOpen(LPHWAVEOUT phwo,
					  UINT_PTR uDeviceID,
					  LPWAVEFORMATEX pwfx,
...
{
	// do open
	MMRESULT ret = old_waveOutOpen(phwo, uDeviceID, pwfx, dwCallback, dwCallbackInstance, fdwOpen);
	// query only?
...
hook_waveOutWrite(HWAVEOUT hwo, LPWAVEHDR pwh, UINT cbwh)
{
	...
	samples		 = av_rescale_rnd(pwh_samples, ga_samplerate, winmm_samplerate, AV_ROUND_UP);
	swr_convert(swrctx, dstplanes, samples, srcplanes, pwh_samples);
