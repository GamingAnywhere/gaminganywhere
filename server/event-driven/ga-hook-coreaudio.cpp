...
static enum AVSampleFormat CA2SWR_format(WAVEFORMATEX* w)
{
...
static int64_t CA2SWR_chlayout(int channels)
{
	if(channels == 1)
...
static int ca_create_swrctx(WAVEFORMATEX* w)
{
...
DllExport HRESULT __stdcall hook_ReleaseBuffer(IAudioRenderClient* thiz, UINT32 NumFramesWritten, DWORD dwFlags)
{
	...
	dstplanes[1] = NULL;
	samples		 = av_rescale_rnd(NumFramesWritten, ga_samplerate, ca_samplerate, AV_ROUND_UP);
	swr_convert(swrctx, dstplanes, samples, srcplanes, NumFramesWritten);
...
#define RET_ON_ERROR(hr, prefix)                                \
	if(hr != S_OK)                                               \
	{                                                            \
		ga_error("[core-audio] %s failed (%08x).\n", prefix, hr); \
		goto hook_ca_quit;                                        \
	}
	hr =
