...
static bool D3D9_screen_capture(IDirect3DDevice9* pDevice)
{
	...
	if(desc.MultiSampleType != D3DMULTISAMPLE_NONE)
	{
		...
		{
			hr = pDevice->CreateRenderTarget(game_width,
														game_height,
														desc.Format,
														D3DMULTISAMPLE_NONE,
		...
		hr = pDevice->StretchRect(renderSurface, NULL, resolvedSurface, NULL, D3DTEXF_NONE);
		if(FAILED(hr))
		{
	...
	{
		hr = pDevice->CreateOffscreenPlainSurface(game_width, game_height, desc.Format, D3DPOOL_SYSTEMMEM, &offscreenSurface, NULL);
		if(FAILED(hr))
		{
	...
	do
	{
		...
		{
			// memcpy(frame->imgbuf+i*encoder_width*sizeof(DWORD), (BYTE *)lockedRect.pBits+i*lockedRect.Pitch,
			// 1*encoder_width*sizeof(DWORD)); CopyMemory(frame->imgbuf, lockedRect.pBits, lockedRect.Pitch * screenRect.bottom);
...
DllExport HRESULT __stdcall hook_D3D9CreateDevice(IDirect3DDevice9* This,
																  UINT Adapter,
...
DllExport HRESULT __stdcall hook_D3D9GetSwapChain(IDirect3DDevice9* This, UINT iSwapChain, IDirect3DSwapChain9** ppSwapChain)
{
...
DllExport HRESULT __stdcall hook_D3D9SwapChainPresent(IDirect3DSwapChain9* This,
...
DllExport HRESULT __stdcall hook_D3D9DevicePresent(IDirect3DDevice9* This,
																	CONST RECT* pSourceRect,
																	CONST RECT* pDestRect,
																	HWND hDestWindowOverride,
																	CONST RGNDATA* pDirtyRegion)
...
#if 1
enum DX_VERSION { dx_none = 0, dx_9, dx_10, dx_10_1, dx_11 };
...
DllExport HRESULT __stdcall hook_DXGICreateSwapChain(IDXGIFactory* This,
																	  IUnknown* pDevice,
																	  DXGI_SWAP_CHAIN_DESC* pDesc,
...
DllExport HRESULT __stdcall hook_D3D10CreateDeviceAndSwapChain(IDXGIAdapter* pAdapter,
...
{
	HRESULT hr =
	  pD3D10CreateDeviceAndSwapChain(pAdapter, DriverType, Software, Flags, SDKVersion, pSwapChainDesc, ppSwapChain, ppDevice);
...
DllExport HRESULT __stdcall hook_D3D10CreateDeviceAndSwapChain1(IDXGIAdapter* pAdapter,
																					 D3D10_DRIVER_TYPE DriverType,
																					 HMODULE Software,
																					 UINT Flags,
...
{
	HRESULT hr = pD3D10CreateDeviceAndSwapChain1(
...
DllExport HRESULT __stdcall hook_D3D11CreateDeviceAndSwapChain(IDXGIAdapter* pAdapter,
...
{
	HRESULT hr = pD3D11CreateDeviceAndSwapChain(pAdapter,
															  DriverType,
															  Software,
															  Flags,
															  pFeatureLevels,
...
DllExport HRESULT __stdcall hook_DXGISwapChainPresent(IDXGISwapChain* This, UINT SyncInterval, UINT Flags)
{
	...
	if(pDESC.BufferDesc.Width != game_width || pDESC.BufferDesc.Height != game_height)
	{
...
void GetRawInputDevice(bool is_mame_game)
{
	...
	for(j = 0; j < nDevices; j++)
	{
		...
		char tBuffer[256] = {0};
		if(GetRawInputDeviceInfo(pRawInputDeviceList[j].hDevice, RIDI_DEVICENAME, tBuffer, &nBufferSize) < 0)
		{
		...
		if(GetRawInputDeviceInfo(pRawInputDeviceList[j].hDevice, RIDI_DEVICEINFO, &rdi, &cbSize) < 0)
		{
		...
		{
#ifdef SHOW_DEVICE_MSG
			ga_error("[device %d][keyboard] mode %ld, number of function keys: %ld, indicators: %ld, keys total: %ld, type: %ld, "
						"subtype: %ld\n",
						j,
...
DllExport HRESULT __stdcall hook_GetRawInputData(HRAWINPUT hRawInput,
																 UINT uiCommand,
																 LPVOID pData,
																 PUINT pcbSize,
																 UINT cbSizeHeader)
	...
	if(uiCommand == RID_INPUT && pData != NULL)
	{
		...
		if(input->data.keyboard.VKey != NULL)
		{
			...
			{ // SendInput
				...
				if(input->data.keyboard.VKey == 17 || input->data.keyboard.VKey == 18 || input->data.keyboard.VKey == 37 ||
					input->data.keyboard.VKey == 38 || input->data.keyboard.VKey == 39 || input->data.keyboard.VKey == 40)
				{
