...
int hook_and_launch(const char* ga_root, const char* config_path, const char* app_exe)
{
	...
	do
	{
		...
		for(ptr = ga_conf_mapkey("game-env", buf, sizeof(buf)); ptr != NULL; ptr = ga_conf_mapnextkey("game-env", buf, sizeof(buf)))
		{
			//
	...
	if(ga_conf_mapsize("game-argv") > 0)
	{
		...
		for(ptr = ga_conf_mapkey("game-argv", buf, sizeof(buf)); ptr != NULL && cmdpos < cmdspace;
			 ptr = ga_conf_mapnextkey("game-argv", buf, sizeof(buf)))
		{
			...
			if(val == NULL)
				continue;
			ga_error("Game arg: %s\n", val);
			n = snprintf(cmdline + cmdpos, cmdspace - cmdpos, " \"%s\"", val);
			cmdpos += n;
		}
