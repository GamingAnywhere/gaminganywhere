...
int hook_SDL_UpperBlit(SDL12_Surface* src, SDL12_Rect* srcrect, SDL12_Surface* dst, SDL12_Rect* dstrect)
{
	return hook_SDL_BlitSurface(src, srcrect, dst, dstrect);
}

static int hook_sdl12(const char* hook_type, const char* hook_method)
{
	HMODULE hMod;
	char audio_type[64] = "";
...
static int hook_sdl2(const char* hook_type, const char* hook_method)
{
	HMODULE hMod, hModGL;
	char audio_type[64] = "";
	// const char *sdlpath = getenv("LIBSDL_SO");
...
static int create_d3d9_object()
{
	...
	hWnd = CreateWindowA(
	  "BUTTON", "Temporary Window", WS_SYSMENU | WS_MINIMIZEBOX, CW_USEDEFAULT, CW_USEDEFAULT, 300, 300, NULL, NULL, NULL, NULL);
	if(hWnd == NULL)
	{
	...
	ga_error("Device information: type: %d, intervals: %d", d3dCaps.DeviceType, d3dCaps.PresentationIntervals);
	//
	...
	hr = pD3D->CreateDevice(D3DADAPTER_DEFAULT, D3DDEVTYPE_HAL, hWnd, dwBehaviorFlags, &d3dpp, &pd3dDevice);
	if(FAILED(hr))
	{
...
static int hook_d9(const char* hook_type, const char* hook_method)
{
	HMODULE hMod;
	...
	// Direct3DCreate9()
	pD3d = (TDirect3DCreate9)GetProcAddress(hMod, "Direct3DCreate9");
	if(pD3d == NULL)
	{
...
static int hook_dxgi(const char* hook_type, const char* hook_method)
{
	HMODULE hMod;
	//
...
static int hook_d10_1(const char* hook_type, const char* hook_method)
{
	HMODULE hMod;
	//
	...
	//
	ga_hook_function("D3D10CreateDeviceAndSwapChain1", pD3D10CreateDeviceAndSwapChain1, hook_D3D10CreateDeviceAndSwapChain1);
	return 0;
}

static int hook_d10(const char* hook_type, const char* hook_method)
{
	HMODULE hMod;
	//
	...
	//
	ga_hook_function("D3D10CreateDeviceAndSwapChain", pD3D10CreateDeviceAndSwapChain, hook_D3D10CreateDeviceAndSwapChain);
	return 0;
}

static int hook_d11(const char* hook_type, const char* hook_method)
{
	HMODULE hMod;
	//
	...
	//
	ga_hook_function("D3D11CreateDeviceAndSwapChain", pD3D11CreateDeviceAndSwapChain, hook_D3D11CreateDeviceAndSwapChain);
	return 0;
}

int do_hook(char* hook_type, int hook_type_sz)
{
...
MODULE MODULE_EXPORT void WINAPI NativeInjectionEntryPoint(REMOTE_ENTRY_INFO* info)
{
	if(hook_proc() < 0)
		exit(-1);
	RhWakeUpProcess();
...
MODULE MODULE_EXPORT BOOL APIENTRY DllMain(HINSTANCE hModule, DWORD fdwReason, LPVOID lpReserved)
{
	/* always return true */
	return TRUE;
}
