...
static void* elf_hook(char const* libpath, void const* unused, char const* name, void const* func)
{
#if 1
	return NULL;
#else
...
static void* hook_dlsym(void* handle, const char* symbol)
{
	...
	//
	if((ph = (struct hookentry*)bsearch(&h, hooks, nhooks, sizeof(struct hookentry), comp_hookentry)) != NULL)
	{
...
int hook_libdl(const char* libpath)
{
	...
	if(handle == NULL)
		return -1;
	//
	original = elf_hook(libpath, LIBRARY_ADDRESS_BY_HANDLE(handle), "dlopen", (const void*)hook_dlopen);
	if(original == NULL)
		return -1;
	//
	original = elf_hook(libpath, LIBRARY_ADDRESS_BY_HANDLE(handle), "dlsym", (const void*)hook_dlsym);
	if(original == NULL)
		return -1;
	//
	return 0;
...
int hook_lib_generic(const char* libpath, void* handle, const char* name, void* func)
{
	...
	if(handle == NULL)
		return -1;
	original = elf_hook(libpath, LIBRARY_ADDRESS_BY_HANDLE(handle), name, (const void*)func);
	if(original == NULL)
		return -1;
