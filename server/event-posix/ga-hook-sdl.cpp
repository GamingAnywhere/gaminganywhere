...
static void sdl_hook_symbols()
{
	...
	sdl_hook_init();
	// for hooking
	old_SDL_Init			= (t_SDL_Init)ga_hook_lookup_or_quit(handle, "SDL_Init");
	old_SDL_SetVideoMode = (t_SDL_SetVideoMode)ga_hook_lookup_or_quit(handle, "SDL_SetVideoMode");
	old_SDL_UpperBlit		= (t_SDL_BlitSurface)ga_hook_lookup_or_quit(handle, "SDL_UpperBlit");
	old_SDL_BlitSurface	= (t_SDL_BlitSurface)ga_hook_lookup(handle, "SDL_BlitSurface");
	...
	old_SDL_Flip			  = (t_SDL_Flip)ga_hook_lookup_or_quit(handle, "SDL_Flip");
	old_SDL_UpdateRect	  = (t_SDL_UpdateRect)ga_hook_lookup_or_quit(handle, "SDL_UpdateRect");
	old_SDL_UpdateRects	  = (t_SDL_UpdateRects)ga_hook_lookup_or_quit(handle, "SDL_UpdateRects");
	old_SDL_GL_SwapBuffers = (t_SDL_GL_SwapBuffers)ga_hook_lookup_or_quit(handle, "SDL_GL_SwapBuffers");
	old_SDL_PollEvent		  = (t_SDL_PollEvent)ga_hook_lookup_or_quit(handle, "SDL_PollEvent");
	old_SDL_WaitEvent		  = (t_SDL_WaitEvent)ga_hook_lookup_or_quit(handle, "SDL_WaitEvent");
	old_SDL_PeepEvents	  = (t_SDL_PeepEvents)ga_hook_lookup_or_quit(handle, "SDL_PeepEvents");
	old_SDL_SetEventFilter = (t_SDL_SetEventFilter)ga_hook_lookup_or_quit(handle, "SDL_SetEventFilter");
	// for internal use
	old_SDL_CreateRGBSurface = (t_SDL_CreateRGBSurface)ga_hook_lookup_or_quit(handle, "SDL_CreateRGBSurface");
	old_SDL_FreeSurface		 = (t_SDL_FreeSurface)ga_hook_lookup_or_quit(handle, "SDL_FreeSurface");
	old_SDL_PushEvent			 = (t_SDL_PushEvent)ga_hook_lookup_or_quit(handle, "SDL_PushEvent");
	//
...
static void sdl12_hook_replay(sdlmsg_t* msg)
{
	...
	switch(msg->msgtype)
	{
		case SDL_EVENT_MSGTYPE_KEYBOARD:
			if((mi = sdl_keymap.find(msgk->sdlkey)) == sdl_keymap.end())
				break;
			sdl12evt.key.type				  = msgk->is_pressed ? SDL12_KEYDOWN : SDL12_KEYUP;
			sdl12evt.key.which			  = 0;
			sdl12evt.key.state			  = msgk->is_pressed ? SDL_PRESSED : SDL_RELEASED;
			...
			if(msgk->unicode == 0)
			{
				...
				if((mu = sdl_unicodemap.find(msgk->sdlkey)) != sdl_unicodemap.end())
				{
					...
					//
					if((msgk->sdlmod & KMOD_SHIFT) && ((ms = sdl_shiftmap.find(mu->second)) != sdl_shiftmap.end()))
					{
		...
		case SDL_EVENT_MSGTYPE_MOUSEKEY:
			sdl12evt.button.type	  = msgm->is_pressed ? SDL12_MOUSEBUTTONDOWN : SDL12_MOUSEBUTTONUP;
			sdl12evt.button.button = msgm->mousebutton > 5 ? msgm->mousebutton - 2 : msgm->mousebutton;
		...
		case SDL_EVENT_MSGTYPE_MOUSEWHEEL:
			...
			sdl12evt.button.button = (((short)msgm->mousey) > 0) ? SDL12_BUTTON_WHEELDOWN : SDL12_BUTTON_WHEELUP;
...
SDL12_Surface* hook_SDL_SetVideoMode(int width, int height, int bpp, uint32_t flags)
{
	...
	if(ret != NULL)
	{
		...
		dupsurface = old_SDL_CreateRGBSurface(SDL12_SWSURFACE, width, height, 32, rmask, gmask, bmask, amask);
		if(dupsurface == NULL)
		{
		...
		do
		{
			...
			evt.active.state = SDL_APPMOUSEFOCUS | SDL_APPINPUTFOCUS | SDL_APPACTIVE;
			evt.active.gain  = 1;
			if(local_filter == NULL)
			{
...
static void hook_SDL_capture_screen(const char* caller)
{
...
static int filter_SDL_Event(SDL12_Event* event)
{
...
int SDL_Init(unsigned int flags) { return hook_SDL_Init(flags); }
...
int SDL_Flip(SDL12_Surface* screen) { return hook_SDL_Flip(screen); }
...
void SDL_GL_SwapBuffers() { hook_SDL_GL_SwapBuffers(); }

int SDL_PollEvent(SDL12_Event* event) { return hook_SDL_PollEvent(event); }

int SDL_WaitEvent(SDL12_Event* event) { return hook_SDL_WaitEvent(event); }
...
void SDL_SetEventFilter(SDL12_EventFilter filter) { hook_SDL_SetEventFilter(filter); }
