...
static void sdl2_hook_symbols()
{
	...
	sdl2_hook_init();
	// for hooking
	old_SDL2_Init				= (t_SDL2_Init)ga_hook_lookup_or_quit(handle, "SDL_Init");
	old_SDL2_CreateWindow	= (t_SDL2_CreateWindow)ga_hook_lookup_or_quit(handle, "SDL_CreateWindow");
	old_SDL2_CreateRenderer = (t_SDL2_CreateRenderer)ga_hook_lookup_or_quit(handle, "SDL_CreateRenderer");
	old_SDL2_CreateTexture	= (t_SDL2_CreateTexture)ga_hook_lookup_or_quit(handle, "SDL_CreateTexture");
	old_SDL2_UpperBlit		= (t_SDL2_UpperBlit)ga_hook_lookup(handle, "SDL_UpperBlit");
#ifndef SDL_BlitSurface
	old_SDL2_BlitSurface = (t_SDL2_BlitSurface)ga_hook_lookup(handle, "SDL_BlitSurface");
	...
	old_SDL2_BlitSurface = old_SDL2_UpperBlit;
#endif
	old_SDL2_GetRendererInfo  = (t_SDL2_GetRendererInfo)ga_hook_lookup_or_quit(handle, "SDL_GetRendererInfo");
	old_SDL2_RenderReadPixels = (t_SDL2_RenderReadPixels)ga_hook_lookup_or_quit(handle, "SDL_RenderReadPixels");
	old_SDL2_RenderPresent	  = (t_SDL2_RenderPresent)ga_hook_lookup_or_quit(handle, "SDL_RenderPresent");
	old_SDL2_GL_SwapWindow	  = (t_SDL2_GL_SwapWindow)ga_hook_lookup_or_quit(handle, "SDL_GL_SwapWindow");
	old_SDL2_PollEvent		  = (t_SDL2_PollEvent)ga_hook_lookup_or_quit(handle, "SDL_PollEvent");
	old_SDL2_WaitEvent		  = (t_SDL2_WaitEvent)ga_hook_lookup_or_quit(handle, "SDL_WaitEvent");
	old_SDL2_PeepEvents		  = (t_SDL2_PeepEvents)ga_hook_lookup_or_quit(handle, "SDL_PeepEvents");
	old_SDL2_SetEventFilter	  = (t_SDL2_SetEventFilter)ga_hook_lookup_or_quit(handle, "SDL_SetEventFilter");
	// for internal use
	old_SDL2_CreateRGBSurface = (t_SDL2_CreateRGBSurface)ga_hook_lookup_or_quit(handle, "SDL_CreateRGBSurface");
	old_SDL2_FreeSurface		  = (t_SDL2_FreeSurface)ga_hook_lookup_or_quit(handle, "SDL_FreeSurface");
	old_SDL2_PushEvent		  = (t_SDL2_PushEvent)ga_hook_lookup_or_quit(handle, "SDL_PushEvent");
	//
...
static void sdl2_hook_replay(sdlmsg_t* msg)
{
	SDL_Event sdl2evt;
	...
	switch(msg->msgtype)
	{
		case SDL_EVENT_MSGTYPE_KEYBOARD:
			sdl2evt.key.type		 = msgk->is_pressed ? SDL_KEYDOWN : SDL_KEYUP;
			sdl2evt.key.timestamp = time(0);
			// sdl2evt.key.windowId?
			sdl2evt.key.state				 = msgk->is_pressed ? SDL_PRESSED : SDL_RELEASED;
		...
		case SDL_EVENT_MSGTYPE_MOUSEKEY:
			sdl2evt.button.type		 = msgm->is_pressed ? SDL_MOUSEBUTTONDOWN : SDL_MOUSEBUTTONUP;
			sdl2evt.button.timestamp = time(0);
			sdl2evt.button.button	 = msgm->mousebutton > 5 ? msgm->mousebutton - 2 : msgm->mousebutton;
	...
	if(ret != NULL && old_SDL2_GetRendererInfo != NULL)
	{
		do
		{
			...
			for(i = 0, pos = 0; i < info.num_texture_formats; i++)
			{
				...
				{
					// not FourCC
					n = snprintf(&formats[pos],
									 sizeof(formats) - pos,
									 " %08x[type=%d,order=%d,layout=%d,bits=%d,bytes=%d]",
									 f,
									 (f >> 24) & 0x0f, /*type*/
									 (f >> 20) & 0x0f, /*order*/
									 (f >> 16) & 0x0f, /*layout*/
...
static void hook_SDL2_capture_screen(const char* caller, SDL_Renderer* renderer)
{
...
static int filter_SDL2_Event(SDL_Event* event)
{
...
int SDL_Init(unsigned int flags) { return hook_SDL2_Init(flags); }
...
void SDL2_RenderPresent(SDL_Renderer* renderer) { old_SDL2_RenderPresent(renderer); }

void SDL_GL_SwapWindow(SDL_Window* window) { hook_SDL2_GL_SwapWindow(window); }

int SDL_PollEvent(SDL_Event* event) { return hook_SDL2_PollEvent(event); }

int SDL_WaitEvent(SDL_Event* event) { return hook_SDL2_WaitEvent(event); }

int SDL_PeepEvents(SDL_Event* event, int numevents, SDL_eventaction action, uint32_t minType, uint32_t maxType)
{
	return hook_SDL2_PeepEvents(event, numevents, action, minType, maxType);
}

void SDL_SetEventFilter(SDL_EventFilter filter, void* userdata) { hook_SDL2_SetEventFilter(filter, userdata); }

__attribute__((constructor)) static void sdl_hook_loaded(void)
{
