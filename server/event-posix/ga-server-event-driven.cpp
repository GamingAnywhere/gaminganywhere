...
int main(int argc, char* argv[])
{
	...
	if(ga_conf_mapsize("game-argv") > 0)
	{
		...
		for(ptr = ga_conf_mapkey("game-argv", buf, sizeof(buf)); ptr != NULL && i + 1 < sizeof(app_arg) / sizeof(char*);
	...
	snprintf(launch_dir, sizeof(launch_dir), "%s/%s", current_path, dirname(argv[0]));

	// get app's info
	if(app_dir[0] == '\0')
	{
		snprintf(app_dir, sizeof(app_dir), "%s", dirname(strdup(app_exe)));
	}
	...
	do
	{
		...
		if((pid = fork()) == 0)
		{
			...
			for(ptr = ga_conf_mapkey("game-env", buf, sizeof(buf)); ptr != NULL;
