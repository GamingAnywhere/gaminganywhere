...
static void sdlaudio_hook_symbols()
{
#ifndef WIN32
	void* handle = NULL;
	...
	// for hooking
	old_SDL2_OpenAudio  = (t_SDL2_OpenAudio)ga_hook_lookup_or_quit(handle, "SDL_OpenAudio");
	old_SDL2_PauseAudio = (t_SDL2_PauseAudio)ga_hook_lookup_or_quit(handle, "SDL_PauseAudio");
	old_SDL2_CloseAudio = (t_SDL2_CloseAudio)ga_hook_lookup_or_quit(handle, "SDL_CloseAudio");
...
static void sdlaudio_dump_audiospec(SDL_AudioSpec* spec)
{
...
static void hook_SDL2_audio_callback(void* userdata, uint8_t* stream, int len)
{
	...
	dstplanes[1] = NULL;
	swr_convert(swrctx, dstplanes, audio_buf_samples, srcplanes, audio_spec.samples);
...
static enum AVSampleFormat SDL2SWR_format(SDL_AudioFormat format)
{
	switch(format)
	{
		case AUDIO_U8:
...
static int64_t SDL2SWR_chlayout(uint8_t channels)
{
	if(channels == 1)
...
int hook_SDL2_OpenAudio(SDL_AudioSpec* desired, SDL_AudioSpec* obtained)
{
	...
	if(ret >= 0)
	{
		...
		audio_buf_samples = av_rescale_rnd(obtained->samples, rtspconf->audio_samplerate, obtained->freq, AV_ROUND_UP);
		bufreq				= av_samples_get_buffer_size(
			 NULL, rtspconf->audio_channels, audio_buf_samples * 2, rtspconf->audio_device_format, 1 /*no-alignment*/);
...
int SDL_OpenAudio(SDL_AudioSpec* desired, SDL_AudioSpec* obtained) { return hook_SDL2_OpenAudio(desired, obtained); }
...
__attribute__((constructor)) static void sdlaudio_hook_loaded(void)
{
