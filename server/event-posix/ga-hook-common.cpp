...
int load_modules()
{
	...
	char hook_audio[64]	  = "";
	//
	snprintf(module_path, sizeof(module_path), BACKSLASHDIR("%s/mod/filter-rgb2yuv", "%smod\\filter-rgb2yuv"), ga_root);
	if((m_filter = ga_load_module(module_path, "filter_RGB2YUV_")) == NULL)
		return -1;
	//
	snprintf(module_path, sizeof(module_path), BACKSLASHDIR("%s/mod/encoder-video", "%smod\\encoder-video"), ga_root);
	...
	if(ga_conf_readbool("enable-audio", 1) != 0)
	{
		//////////////////////////
#ifndef __APPLE__
		if(ga_conf_readv("hook-audio", hook_audio, sizeof(hook_audio)) == NULL || hook_audio[0] == '\0')
		{
			//////////////////////////
			snprintf(module_path, sizeof(module_path), BACKSLASHDIR("%s/mod/asource-system", "%smod\\asource-system"), ga_root);
			if((m_asource = ga_load_module(module_path, "asource_")) == NULL)
				return -1;
			//////////////////////////
		}
#endif
		snprintf(module_path, sizeof(module_path), BACKSLASHDIR("%s/mod/encoder-audio", "%smod\\encoder-audio"), ga_root);
	...
	{
		snprintf(module_path, sizeof(module_path), BACKSLASHDIR("%s/mod/ctrl-sdl", "%smod\\ctrl-sdl"), ga_root);
		if((m_ctrl = ga_load_module(module_path, "sdlmsg_replay_")) == NULL)
			return -1;
	}
	//////////////////////////
	snprintf(module_path, sizeof(module_path), BACKSLASHDIR("%s/mod/server-live555", "%smod\\server-live555"), ga_root);
