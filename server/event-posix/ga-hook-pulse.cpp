...
static void pulse_hook_symbols()
{
#ifndef WIN32
	...
	// simple
	old_pa_simple_new	  = (t_pa_simple_new)ga_hook_lookup_or_quit(hSimple, "pa_simple_new");
	old_pa_simple_free  = (t_pa_simple_free)ga_hook_lookup_or_quit(hSimple, "pa_simple_free");
	old_pa_simple_write = (t_pa_simple_write)ga_hook_lookup_or_quit(hSimple, "pa_simple_write");
	// async
	old_pa_stream_new					  = (t_pa_stream_new)ga_hook_lookup_or_quit(hPulse, "pa_stream_new");
	old_pa_stream_new_extended		  = (t_pa_stream_new_extended)ga_hook_lookup_or_quit(hPulse, "pa_stream_new_extended");
	old_pa_stream_new_with_proplist = (t_pa_stream_new_with_proplist)ga_hook_lookup_or_quit(hPulse, "pa_stream_new_with_proplist");
	old_pa_stream_connect_playback  = (t_pa_stream_connect_playback)ga_hook_lookup_or_quit(hPulse, "pa_stream_connect_playback");
	old_pa_stream_set_write_callback =
	  (t_pa_stream_set_write_callback)ga_hook_lookup_or_quit(hPulse, "pa_stream_set_write_callback");
	old_pa_stream_write = (t_pa_stream_write)ga_hook_lookup_or_quit(hPulse, "pa_stream_write");
	// register via dlsym?
...
__attribute__((constructor)) static void pulse_hook_loaded(void)
{
...
static enum AVSampleFormat PA2SWR_format(pa_sample_format_t format)
{
	switch(format)
	{
		case PA_SAMPLE_U8:
...
static int64_t PA2SWR_chlayout(uint8_t channels)
{
	if(channels == 1)
...
static int pa_create_swrctx(pa_sample_format_t format, int freq, int channels)
{
	...
	samples				  = av_rescale_rnd(PA_MAX_SAMPLES, rtspconf->audio_samplerate, freq, AV_ROUND_UP);
	bufreq =
	  av_samples_get_buffer_size(NULL, rtspconf->audio_channels, samples * 2, rtspconf->audio_device_format, 1 /*no-alignment*/);
	if((audio_buf = (unsigned char*)malloc(bufreq)) == NULL)
	{
		ga_error("PulseAudio: cannot allocate resample memory.\n");
		return -1;
	}
	if(audio_source_setup(bufreq, rtspconf->audio_samplerate, 16 /* depends on format */, rtspconf->audio_channels) < 0)
	{
...
int hook_pa_simple_write(pa_simple* s, const void* data, size_t bytes, int* error)
{
...
pa_stream* hook_pa_stream_new(pa_context* c, const char* name, const pa_sample_spec* ss, const pa_channel_map* map)
{
	pa_stream* ret;
												...
												pa_stream* sync_stream)
{
	...
	if(ret >= 0)
	{
		...
		{
			pa_create_swrctx(mi->second.format, mi->second.rate, mi->second.channels);
		}
	}
	return ret;
}

void hook_pa_stream_set_write_callback(pa_stream* p, pa_stream_request_cb_t cb, void* userdata)
{
...
int hook_pa_stream_write(pa_stream* p, const void* data, size_t nbytes, pa_free_cb_t free_cb, int64_t offset, pa_seek_mode_t seek)
{
	//
	...
	if(old_pa_stream_write == NULL)
		pulse_hook_symbols();
	//
	if(p == pa_stream_main && nbytes > 0 && offset == 0 && seek == PA_SEEK_RELATIVE)
		do
		{
			...
			dstsamples	 = av_rescale_rnd(srcsamples, ga_samplerate, pa_samplerate, AV_ROUND_UP);
			swr_convert(swrctx, dstplanes, dstsamples, srcplanes, srcsamples);
