...
static void sdlaudio_hook_symbols()
{
	...
	// for hooking
	old_SDL_OpenAudio	 = (t_SDL_OpenAudio)ga_hook_lookup_or_quit(handle, "SDL_OpenAudio");
	old_SDL_PauseAudio = (t_SDL_PauseAudio)ga_hook_lookup_or_quit(handle, "SDL_PauseAudio");
	old_SDL_CloseAudio = (t_SDL_CloseAudio)ga_hook_lookup_or_quit(handle, "SDL_CloseAudio");
...
static void sdlaudio_dump_audiospec(SDL12_AudioSpec* spec)
{
...
static void hook_SDL_audio_callback(void* userdata, uint8_t* stream, int len)
{
...
static enum AVSampleFormat SDL2SWR_format(uint16_t format)
{
	switch(format)
	{
		case SDL12_AUDIO_U8:
...
int SDL_OpenAudio(SDL12_AudioSpec* desired, SDL12_AudioSpec* obtained) { return hook_SDL_OpenAudio(desired, obtained); }
